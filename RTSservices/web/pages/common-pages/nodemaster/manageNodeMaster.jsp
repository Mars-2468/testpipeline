...
<div class="mainHdr">
	<h3> 
		<c:choose>
			<c:when test="${node.nodeMasterId==0}"><fmt:message key="node.manage.pagetitle.new" /></c:when>
...
<tr>
<td colspan="2">
<spring:bind path="node.nodeMasterId">
...
function saveNode(frm)
{
	...
	if(checkMandatoryDetailed(arrRequiredElements,arrRequiredElementsMessages))
		...
		{		
			navigate(frm,'<c:out value="${contextRoot}"/>/nodemaster/saveNodeMaster.do');
		}
		}
}
...
{
if(confirmDelete()){
	onPageSubmit('<c:out value="${contextRoot}"/>/nodemaster/deleteNodeMaster.do');
}
}
</script>