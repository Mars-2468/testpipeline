...
function editNode(id)
{
	...
	onPageSubmit('<c:out value="${contextRoot}"/>/node/editNode.do');
}
function searchNode()
{
	onPageSubmit('<c:out value="${contextRoot}"/>/node/listNode.do');
}
</script>
<div class="mainHdr">
	<h3> <a href="<c:out value="${contextRoot}"/>/node/editNode.do">
			...
			<tr>
				...
				<td>
					<input type="text" class="ClsTextbox" id="searchName" maxlength="50" size="50" 
						style="width:110px" name="searchName" 
...
<tr>
<td><a title="<fmt:message key="list.link.EditOrViewLink"/>" href="javascript:editNode('${NodeList.nodeId}');">
<c:out value="${NodeList.name}"/></a></td>
