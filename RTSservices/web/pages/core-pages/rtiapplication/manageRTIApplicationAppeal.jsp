...
%>
 ...
 function getApplicationDetails(rtiApplicationNumber)
 {
	// CommonServiceDWR.getRTIApplicationDetails(rtiApplicationNumber,setApplicationDetails);
 }
 function setApplicationDetails(rtiApplicationData)
 {
	 if(rtiApplicationData != null){
		//rtiApplicationRefId applDept applicationDepartmentId applicantName gender bplVal bplCardNumber
	 }else{
		 displayError("RTI Application Not Found. Make Sure You Entered Correct RTi Application Number");
	 }
 }
...
<table width="98%" border="0" cellpadding="2" cellspacing="2" class="dataForm">
	...
	<tr>
		...
		<td >
			<span class="ClsLabel"> RTI Reference No.</span></td>
		<td>
		<%-- 	<c:choose>
				<c:when test="${rtiApplication.rtiApplicationId!=0}">
					<spring:bind path="rtiApplication.rtiApplicationRefNumber">
						 <input type="text" class="ClsTextbox"  id="rtiApplicationRefNumber" name="rtiApplicationRefNumber" onblur="getApplicationDetails(this.value)" />
					</spring:bind>
				</c:when>
				<c:otherwise>
					<spring:bind path="rtiApplication.rtiApplicationRefNumber">
						 <input type="text" class="ClsTextbox"  id="rtiApplicationRefNumber" name="rtiApplicationRefNumber" readonly="readonly" value="${rtiApplication.rtiApplicationRefNumber}" />
					</spring:bind>
				</c:otherwise>
			</c:choose> --%>
			<spring:bind path="rtiApplication.rtiApplicationRefNumber">
			...
			<spring:bind path="rtiApplication.rtiApplicationRefId">
				 <input type="hidden" readonly="readonly"  id="rtiApplicationRefId" name="rtiApplicationRefId" value="${rtiApplication.rtiApplicationRefId}" />
			</spring:bind>
			<spring:bind path="rtiApplication.isRTIAppealApplication">
				 <input type="hidden" readonly="readonly" id="isRTIAppealApplication" name="isRTIAppealApplication" value="1" />
			</spring:bind>
		</td>
		<td >
			<span class="ClsLabel"> Related Inward Number </span><span class="ClsRequiredFields">*</span></td>
		<td>
			<spring:bind path="rtiApplication.inWardNumber">
	 ...
	 <c:if test="${rtiApplication.rtiApplicationId!=0}">
		<tr>
			...
			<td >
				<span class="ClsLabel">Computer Number: </span>
			</td>
			<td>				
				 ${rtiApplication.rtiApplicationId}
			</td>
		</tr>
	</c:if> 
	...
	<tr>
		...
		<td>
			<spring:bind path="rtiApplication.registrationDate">
				 <input type="text" class="ClsTextboxDate" readonly="readonly"  id="registrationDate" name="registrationDate" onchange="javascript:getFeeDetails();"  value="${rtiApplication.registrationDate}"  />
			</spring:bind>
		</td>
	</tr> 
	<tr>
		...
		<td>
			 ...
			 <input type="text" class="ClsTextbox" readonly="readonly"  id="applDept" name="applDept"  value="${rtiApplication.applicationDepartment.name}"  />	
			 <input type="hidden"  readonly="readonly"  id="applicationDepartmentId" name="applicationDepartmentId"  value="${rtiApplication.applicationDepartment.departmentId}"  />	
	...
	<tr>
		...
		<td>
			<spring:bind path="rtiApplication.informationRequired">
				<textarea class="ClsTextbox" id="informationRequired"   cols="40" rows="4" name="informationRequired">
					<c:out value="${rtiApplication.informationRequired}" /></textarea>
			</spring:bind>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			<spring:bind path="rtiApplication.applicantName">
			<input type="text" class="ClsTextbox" readonly="readonly" id="applicantName" maxlength="45"   placeholder="Enter Name" name="applicantName" 
				value="<c:out value="${rtiApplication.applicantName}"/>" />
			</spring:bind>
		</td>
		<td >
		...
		<td >			
			<spring:bind path="rtiApplication.gender">
				<input type="radio" id="gender${pageScope.LABEL_RADIO_MALE}" name="gender" value="${pageScope.RADIO_MALE}" readonly="readonly"  <c:if test="${rtiApplication.gender eq pageScope.RADIO_MALE}">checked="checked"</c:if>  />
				<label for="gender${pageScope.LABEL_RADIO_MALE}">${pageScope.LABEL_RADIO_MALE} </label>
				<input type="radio" id="gender${pageScope.LABEL_RADIO_FEMALE}" name="gender" value="${pageScope.RADIO_FEMALE}"  readonly="readonly"   <c:if test="${rtiApplication.gender eq pageScope.RADIO_MALE}">checked="checked"</c:if> />
				<label for="gender${pageScope.LABEL_RADIO_FEMALE}" >${pageScope.LABEL_RADIO_FEMALE}</label>
			</spring:bind>
		</td>
	</tr>
	<tr>
		...
		<td colspan="3">			
			<spring:bind path="rtiApplication.bplCardNumber">
				<input type="text" class="ClsTextbox" id="bplCardNumber" maxlength="45" readonly="readonly"   placeholder="BPL Card Number" name="bplCardNumber" style="display:none"
					value="<c:out value="${rtiApplication.bplCardNumber}"/>" />
			</spring:bind>		 
			<input type="checkbox" id="bpl" value="0" readonly="readonly"  name="bplVal" onclick="javascript:getFeeDetails();showOrHideCardNumber();" <c:if test="${rtiApplication.bpl == 1}">checked="checked"</c:if>  /> 
	...
	<tr>
		...
		<td>
			<spring:bind path="rtiApplication.addressLine1">
				<input  type="text" class="ClsTextbox" id="addressLine1" readonly="readonly"   name="addressLine1" value="${rtiApplication.addressLine1}">
			</spring:bind>
		</td>
		<td>
		...
		<td>
			<spring:bind path="rtiApplication.addressLine2">
				<input  type="text" class="ClsTextbox" id="addressLine2" readonly="readonly"   name="addressLine2" value="${rtiApplication.addressLine2}">
			</spring:bind>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			...
			<input type="text" class="ClsTextbox" id="state" maxlength="45" readonly="readonly"  name="state"  readonly="readonly" value="<c:out value="${rtiApplication.state}"/>" />
			</spring:bind>
		</td>
	    <td >
		...
		<td>
			...
			<input type="text" class="ClsTextbox" id="district"  name="district" readonly="readonly"    placeholder="District"  
	...
	<tr>
		...
		<td>
			...
			<input type="text" class="ClsTextbox" id="pincode" maxlength="6" readonly="readonly"  name="pincode"   placeholder="Enter Pincode"  
		...
		<td>
			...
			<input type="text" class="ClsTextbox" id="phoneNumber" maxlength="10"  readonly="readonly"  name="phoneNumber" placeholder="XXXXXXXXXX" 
	...
	<tr>
		...
		<td>
			...
			<input type="text" class="ClsTextbox" id="mobileNumber" maxlength="10"   readonly="readonly"  name="mobileNumber" placeholder="XXXXXXXXXX" 
		...
		<td>
			...
			<input type="text" class="ClsTextbox" id="email" name="email" readonly="readonly" maxlength="45" placeholder="e.g user@gmail.com" 
		...
		<td>
			<spring:bind path="rtiApplication.numberOfCopies">
				<input type="text" class="ClsTextbox"  id="numberOfCopies" name="numberOfCopies" maxlength="10" onblur="javascript:getFeeDetails();"  
				 value="${rtiApplication.numberOfCopies}"  />
			</spring:bind>
		</td>
		<td>
			<span class="ClsLabel">Amount Per Copy</span></td>
		<td>
			 <spring:bind path="rtiApplication.amountPerCopy">
				<input type="text" class="ClsTextbox" id="amountPerCopy" name="amountPerCopy" maxlength="2"  readonly="readonly"  value="${rtiApplication.amountPerCopy}"  />
			</spring:bind>
		</td>
	</tr> 
	...
	<tr>		
		...
		<td>
			...
			<input type="text" class="ClsTextbox" id="postalCharges" name="postalCharges"" maxlength="16" readonly="readonly" style="border:none;" 
			 value="${rtiApplication.postalCharges}" />
	...
	<tr> 
		...
		<td> 
			 	<input type="checkbox" id="iSFeePaidVal"  name="iSFeePaidVal"  <c:if test="${rtiApplication.isFeePaid == 1}">checked="checked"</c:if> onclick="showPayMode()" /> 
 		</td>
	...
	<tr> 
		<td colspan="4">
			<div id="payModeDetails">
				<input type="hidden"  id="collectionId"  readonly="readonly"  name="collectionId" value="${rtiApplication.collection.collectionId}" />
				<table  cellpadding="2" cellspacing="2" width="100%" class="dataForm" > 
					<tr>
						<td><span class="ClsLabel" >Receipt Number</span></td>	
						<td>
							<input type="text" class="ClsTextbox" id="receiptNumber"  maxlength="15"    name="receiptNumber" value="${rtiApplication.collection.receiptNumber}" />	
						</td>
						<td><span class="ClsLabel" >Receipt Date</span></td>	
						<td> <input type="text" class="ClsTextboxDate" id="receiptDate"  maxlength="10" readonly="readonly" name="receiptDate" value="${rtiApplication.collection.receiptDate}" />	 </td>	 
					</tr>
					<tr id="chequeDetails">
						<td><span class="ClsLabel" id="chqDD_No_lbl">Cheque Number</span><span class="ClsRequiredFields">*</span></td>	
						<td>
							<input type="text" class="ClsTextbox" id="chequeDDRTGSNumber"   name="chequeDDRTGSNumber" value="${rtiApplication.collection.chequeDDRTGSNumber}" />	
						</td>					
						<td><span class="ClsLabel" id="chqDD_Date_lbl">Cheque Date</span><span class="ClsRequiredFields">*</span></td> 
						 <td>
							<input type="text" class="ClsTextboxDate" id="chequeDDRTGSDate"  maxlength="10" readonly="readonly" name="chequeDDRTGSDate" value="${rtiApplication.collection.chequeDDRTGSDate}" />
						</td>
					</tr>
					<tr id="bankDetails"> 
						<td><span class="ClsLabel">Bank Name<span class="ClsRequiredFields">*</span></span></td>
						<td>
							<input type="text" class="ClsTextbox"  name="bankName" id="bankName" maxlength="50"  value="${rtiApplication.collection.bankName}" />
						</td>
						<td><span class="ClsLabel">Branch Name<span class="ClsRequiredFields">*</span></span></td>	
						<td>
							<input type="text" class="ClsTextbox"  name="branchName" maxlength="50"  id="branchName"  value="${rtiApplication.collection.branchName}" />
						</td>
					</tr>
					<tr> 
						<td><span class="ClsLabel"  id="chqDD_Amount_lbl">Amount<span class="ClsRequiredFields">*</span></span></td>
						<td colspan="3">
							<input type="text" class="ClsTextbox"  name="amountPaid" id="amountPaid" maxlength="5"  value="${rtiApplication.collection.amountPaid}" />
						</td>						
					</tr>
				</table>
			</div>			
	...
	<tr><td></td>
	 ...
	 <div id="isParked">
		<c:if test="${not empty rtiApplication && rtiApplication.rtiApplicationId != 0 && not empty rtiApplication.rtiApplicationDetails }">			 
			<%@include file="/pages/core-pages/rtiapplication/listRTIApplicationVerificationDetails.jsp"%>	 
		</c:if>
			<br/>
		<c:if test="${not empty rtiApplication && rtiApplication.rtiApplicationId != 0 }">
...
<c:if test="${(not empty rtiApplication.isApplicationParked && rtiApplication.isApplicationParked == 1 ) || (rtiApplication.finalStatus eq pageScope.APPL_STATUS_DISCARDED) }">
	<script>
								...
								//if(isValidDate(new Array('registrationDate'),new Array('Registration Date'))){		
									...
									if(document.getElementById("bpl").checked==true){										
										if(checkMandatoryDetailed(new Array('bplCardNumber'),new Array('BPL Card Number'))){
											if(validateFeePaid()){
												navigate(frm,'<c:out value="${contextRoot}"/>/rtiapplicationAppeal/saveRTIApplicationAppeal.do');
											}
										}
									}else{										
										if(validateFeePaid()){
