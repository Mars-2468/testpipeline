...
//alert('${requestScope.workFlowTask.name}');
	...
	{
		document.getElementById('manageRTIApplicationId').value=id;
		document.getElementById('actionTaken').value='acceptTask';
		onPageSubmit('<c:out value="${contextRoot}"/>/rtiapplicationReAssigned/acceptAssignedTask.do');
	}
	function rejectTask(id)
	{
		document.getElementById('manageRTIApplicationId').value=id;
		document.getElementById('actionTaken').value='rejectTask';
		onPageSubmit('<c:out value="${contextRoot}"/>/rtiapplication/editRTIApplicationPark.do');
	}
...
<c:if test="${not empty rtiApplication.isRTIAppealApplication && rtiApplication.isRTIAppealApplication==1}">
	
</c:if> 

  <c:if test="${ (empty rtiApplication.isApplicationParked || rtiApplication.isApplicationParked == 0 )
  				&& (not empty rtiApplication.hasAssignedUser  && rtiApplication.hasAssignedUser == 1)}">
...
<c:if test="${rtiApplication.rtiApplicationId!=0 && rtiApplication.hasAssignedUser==1}">	
	<div id="actionMenuId"  >		 
  ...
  <c:if test="${rtiApplication.rtiApplicationId!=0 && rtiApplication.hasAssignedUser==0 }">
  	<c:choose>
  		<c:when test="${rtiApplicationReAssignAcceptance.assignedTo.userId == SessionUser.userId}">
	  		<div id="actionMenuId" style="background-color: #fff000">  	    
		  		Application is RE-Assigned to you for further action &nbsp;&nbsp;
		  		<button type="button" class="btn btn-primary btn-xs" onclick="acceptTask('${rtiApplication.rtiApplicationId}')"  >Accept</button>
				<button type="button" class="btn btn-primary btn-xs" onclick="rejectTask('${rtiApplication.rtiApplicationId}')" >Reject</button>
  		...
  		<c:otherwise>
  			<div id="actionMenuId" style="background-color: #fff000">  
		  		You do not have a permission, workflow is Re-Assigned to-  &nbsp; ${rtiApplicationReAssignAcceptance.assignedTo.firstName} ${rtiApplicationReAssignAcceptance.assignedTo.lastName} 
			</div>	
  		</c:otherwise>
  	</c:choose>
...
<table width="98%" border="0" cellpadding="2" cellspacing="2" class="dataForm">
		...
		<td>
			<spring:bind path="rtiApplication.inWardNumber">
			 ...
			 <spring:bind path="rtiApplication.isRTIAppealApplication">
				 <input type="hidden" readonly="readonly"  id="isRTIAppealApplication" name="isRTIAppealApplication" value="0" />
			</spring:bind>
		</td>
		<td >
	...
	<tr>
		...
		<td>
			<spring:bind path="rtiApplication.receivedRefNo">
	...
	<tr>
		...
		<td>
			<spring:bind path="rtiApplication.subject">
				<textarea class="ClsTextbox" id="subject"   cols="40" rows="4" name="subject">${rtiApplication.subject}</textarea>
			</spring:bind>
		</td>
		<td>
		...
		<td>
			<spring:bind path="rtiApplication.informationRequired">
				<textarea class="ClsTextbox" id="informationRequired"   cols="40" rows="4" name="informationRequired">${rtiApplication.informationRequired}</textarea>
			</spring:bind>
		</td>
	</tr>	
	...
	<tr>
		...
		<td>
			...
			<input type="text" class="ClsTextbox" id="applicantName" maxlength="45"   placeholder="Enter Name" name="applicantName" 
		...
		<td >
			<spring:bind path="rtiApplication.gender">
				<input type="radio" id="gender${pageScope.LABEL_RADIO_MALE}" name="gender" value="${pageScope.RADIO_MALE}"  <c:if test="${rtiApplication.gender eq pageScope.RADIO_MALE}">checked="checked"</c:if>  />
				<label for="gender${pageScope.LABEL_RADIO_MALE}">${pageScope.LABEL_RADIO_MALE} </label>
				<input type="radio" id="gender${pageScope.LABEL_RADIO_FEMALE}" name="gender" value="${pageScope.RADIO_FEMALE}"   <c:if test="${rtiApplication.gender eq pageScope.RADIO_MALE}">checked="checked"</c:if> />
	...
	<tr>
		...
		<td colspan="3">			
			<spring:bind path="rtiApplication.bplCardNumber">
				<input type="text" class="ClsTextbox" id="bplCardNumber" maxlength="45"  placeholder="BPL Card Number" name="bplCardNumber" style="display:none"
					value="<c:out value="${rtiApplication.bplCardNumber}"/>" />
			</spring:bind>		 
			<input type="checkbox" id="bpl" value="0"  name="bplVal" onclick="javascript:getFeeDetails();showOrHideCardNumber();" <c:if test="${rtiApplication.bpl == 1}">checked="checked"</c:if>  /> 
	...
	<tr>
		...
		<td>
			...
			<input type="text" class="ClsTextbox" id="state" maxlength="45" name="state"  readonly="readonly" style="width:300px" value="<c:out value="${rtiApplication.state}"/>" />
			</spring:bind>
		</td>
	    <td >
	...
	<c:if test="${empty rtiApplication.rtiApplicationDetails}">	
		<tr><td></td>
...
<c:if test="${(not empty rtiApplication.isApplicationParked && rtiApplication.isApplicationParked == 1 ) 
	|| (rtiApplication.finalStatus eq pageScope.APPL_STATUS_DISCARDED) 
	|| (rtiApplication.hasAssignedUser == 0)}">
	<script>
