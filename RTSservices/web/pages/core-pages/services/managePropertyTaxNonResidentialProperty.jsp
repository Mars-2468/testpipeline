...
<div class="mainHdr">
	<h3>
		...
		<fmt:message key="Exemption in Tax if Citizen is Non-Residential" />
		:
...
<td colspan="1.5"><span class="ClsLabel" style="font-size: 14px"><fmt:message
			...
			<td colspan="1.5"><span class="ClsLabel" style="font-size: 14px"><fmt:message
						key="Total Demand (एकूण मागणी)" />:</span></td>
			<td colspan="2">

					<input type="text" class="form-control" style="width: 180px"
						id="childeName" maxlength="45" tabindex="3" name="ownerName"
						value="<c:out value="${propertyTaxAssessment.total_demand}"/>" disabled/>
			</td>
...
<tr>
		...
		<td colspan="1.5"><span class="ClsLabel" style="font-size: 14px"><fmt:message
			...
			<td colspan="2"><input type="text" class="form-control"
				...
				name="fees"
				value="<c:out value="${propertyTaxAssessment.fees}"/>"
				disabled /></td>
</tr>
			...
			<tr>
				<td colspan="3.5"><span class="ClsLabel"
					style="font-size: 14px"><fmt:message
							key="Application in Prescribed format (विहित नमुन्यातील अर्ज)" />:</span> 
   ...
   <span class="download" style="align-items: center;color:white;">Download</span>
		...
		<c:if test="${doc[1] != 'null'}">
			<tr>
				<td colspan="3.5"><span class="ClsLabel"
					style="font-size: 14px"><fmt:message
							key="No Due Certificate (मालमत्ता कर भरणा पावती)" />:</span> 
				<td style="text-align: center" colspan="3.5"><a class="bg-button btn btn-primary bd-highlight action"
   type="button"
   target="_blank"
   ...
   <span class="download" style="align-items: center;color:white;">Download</span>
		...
		<!--<c:if test="${doc[2] != 'null'}">
			<tr>
				<td colspan="3.5"><span class="ClsLabel"
					style="font-size: 14px"><fmt:message
							key="Copy of Electric Bill
							(विद्युत बिलाची प्रत)" />:</span> 
				<td style="text-align: center" colspan="3.5"><a
					class="ClsButton" type="button" target="_blank"
					href="<c:out value="${doc[2]}"/>"><span
...
<script type="text/javascript">
	function saveEntity() {
		onPageSubmit('<c:out value="${contextRoot}"/>/rtiapplication/createPropertyTaxNonResidentialWork.do');
	}
	...
	function saveFees(){
		...
		onPageSubmit('<c:out value="${contextRoot}"/>/rtsapplication/savePropertyTaxNonResidentialFees.do?id='+refId);
		}else{
			return false;
		}
		}else{
