...
<script type="text/javascript">
	function editRTIApplication(id) {
		...
		onPageSubmit('<c:out value="${contextRoot}"/>/rtiapplication/editGaneshMandapPermission.do');
	}
	function searchRTIApplication() {
		onPageSubmit('<c:out value="${contextRoot}"/>/rtsapplication/listRTSGaneshMandapApplication.do');
	}
</script>
<style type="text/css">
.btn-success { -
...
<c:choose>
                        <c:when test="${rtiApplnList.applicationCost==0.0}">
...
<c:if test="${rtiApplnList.workFlowStatus==9}">
						Citizen Intial Payment Completed
					</c:if>
...
<input type="hidden" id="rtiApplicationRefId" name="rtiApplicationRefId" value="<c:out value="${rtiApplnList.rtiApplicationRefId}" />" />
<input type="hidden" id="serviceId" name="serviceId" value="<c:out value="${rtiApplnList.rtiserviceid}" />" />
