...
<script type="text/javascript">
	...
	function searchRTIApplication() {
		onPageSubmit('<c:out value="${contextRoot}"/>/rtsapplication/listRTSTreeCuting.do"');
	}
</script>
<script type="text/javascript">

function downloadReceipt(applicationNumber) {
    ...
    var url = '<c:out value="${contextRoot}"/>/ws/rtsapplication/printRecieptDownload.do'
            + '?rtiApplnNumber=' + encodeURIComponent(applicationNumber);
...
<c:if
					...
					<c:if test="${rtiApplnList.workFlowStatus==9 && rtiApplnList.finalStatus != 'L2'}">
					...
					<c:if
						test="${rtiApplnList.workFlowStatus==11}">
						<b>At L4 level</b>
					</c:if> 
					<c:if test="${rtiApplnList.workFlowStatus==3}">
						<b>Citizen Payment Pending. </b>
					</c:if> <c:if test="${rtiApplnList.workFlowStatus==2}">
						<b>Citizen Payment Completed
						 <button type="button"
        class="receipt-btn"
        onclick="downloadReceipt('<c:out value="${rtiApplnList.rtiApplnNumber}"/>')">
    <i class="bi bi-download"></i>
    Download Receipt
</button>
</b>
					</c:if> <c:if test="${rtiApplnList.workFlowStatus==1}">
						<b>Completed and File Uploaded for Citizen
						 <button type="button"
        class="receipt-btn"
        onclick="downloadReceipt('<c:out value="${rtiApplnList.rtiApplnNumber}"/>')">
    <i class="bi bi-download"></i>
...
<c:if
					...
					<c:if test="${rtiApplnList.workFlowStatus==3}">
						...
						<b>Citizen Payment Completed<br>
    ...
    <h6 style="margin-top: 3px;">Download Receipt</h6>
						...
						<b>Completed and File Uploaded for Citizen<br>
						<button class="btn btn-success btn_sm"
        type="button"
        style="height: 40px;"
        onclick="downloadReceipt('<c:out value="${rtiApplnList.rtiApplnNumber}"/>')">
