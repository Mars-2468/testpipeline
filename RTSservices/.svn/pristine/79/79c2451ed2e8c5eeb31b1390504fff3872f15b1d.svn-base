 ...
 * <p>Title: WorkflowTemplateDaoImpl.java </p>
...
public class WorkflowTemplateDAOImpl extends HibernateDaoSupport implements WorkflowTemplateDAO {

    private static final Log log = LogFactory.getLog(WorkflowTemplateDAOImpl.class);
	...
	public WorkflowTemplate get(long id) {
		try {
			...
			return (WorkflowTemplate) getHibernateTemplate().get(WorkflowTemplate.class,new Long(id));
    ...
    public WorkflowTemplate merge(WorkflowTemplate entity) {
    	try {
      		...
      		if (entity.getWorkflowTemplateId() > 0) {
      			WorkflowTemplate workflowTemplate = get(entity.getWorkflowTemplateId());
      			getHibernateTemplate().evict(workflowTemplate);
      			getHibernateTemplate().flush();
      		}

		    return (WorkflowTemplate) getHibernateTemplate().merge(entity);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace(),exception);
	    	exception.printStackTrace();
    ...
    public void delete(long id) {
        try {
    		if(log.isDebugEnabled()){
        		log.debug("deleting WorkflowTemplate instance");
        	}
        	WorkflowTemplate entity = (WorkflowTemplate) getHibernateTemplate().load(WorkflowTemplate.class, new Long(id));
	...
	public List<WorkflowTemplate> getAll() {
		try {
			if (log.isDebugEnabled()) {
				log.debug("get all Method is called for WorkflowTemplate");
			}
			return getHibernateTemplate().loadAll(WorkflowTemplate.class);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace(),exception);
	    	exception.printStackTrace();
    ...
    public void deleteAll(List<WorkflowTemplate> listWorkflowTemplate)
    {
		try
		{
	    	if (log.isDebugEnabled())
	   		{
				log.debug("deleteAll Method is called for WorkflowTemplate");
	    	}
	    	getHibernateTemplate().deleteAll(listWorkflowTemplate);
	    	log.debug("deleted successfuly a list of WorkflowTemplate records from database");
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace(),exception);
	    	exception.printStackTrace();
    ...
    public void saveAll(List<WorkflowTemplate> listWorkflowTemplate)
    {
		try
		{
	    	...
	    	getHibernateTemplate().saveOrUpdateAll(listWorkflowTemplate);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace(),exception);
	    	exception.printStackTrace();
	...
	public List<WorkflowTemplate> getWorkflowTemplateList(long pageNumber,String orderBy, String sortBy)
	{
		try {
			...
			String fromClause = "select workflowTemplate from WorkflowTemplate workflowTemplate ";
			...
			List<WorkflowTemplate> list = query.list();
	...
	public long getWorkflowTemplateCount(){
	    try {
			String fromClause = "select count(*) from WorkflowTemplate workflowTemplate";
	...
	public List<WorkflowTemplate> getEntityListBySQLQuery(String strSql, Hashtable<String, NullableType> hstDataTypes)
	{
		try{
			...
			query.addEntity(WorkflowTemplate.class);
			return query.list();
		} catch (Exception exception)
		{
			log.error(exception.getStackTrace(),exception);
	...
	public List<WorkflowTemplate> findByProperty(String propertyName, Object value) 
    {
		try 
	  	{
		  	log.info("finding WorkflowTemplate instance with property: "+ propertyName + ", value: " + value);
			String queryString = "select workflowTemplate from WorkflowTemplate workflowTemplate where workflowTemplate."+ propertyName + "= ?";
		    Query queryObject = getSession().createQuery(queryString);
		    queryObject.setParameter(0, value);
		   // CommonUtils.setFilter(getSession(), 1);
	   		return queryObject.list();
		} catch (Exception exception)
		{
			log.error(exception.getStackTrace(),exception);
	...
	public WorkflowTemplate checkForDuplicates(WorkflowTemplate workflowTemplate)
	{
		...
		try {
			String fromClause = "select workflowTemplate from WorkflowTemplate workflowTemplate where " ;

			if(workflowTemplate.getWorkflowTemplateId() > 0)		
				fromClause = fromClause +" workflowTemplateId <> :workflowTemplateId and ";
			
			//Sample Code update accordingly
			//fromClause = fromClause +" workflowTemplate.{Many-to-one}.{Many-to-one}Id = "+workflowTemplate.get{Many-to-one}().get{Many-to-one}Id()+" and ";
			//fromClause = fromClause +" (upper(name) = upper('"+ CommonUtils.getEscapedSQLString(workflowTemplate.getName()) +"')) " ;
			
			fromClause = fromClause +" upper(workflowTemplateName) = :workflowTemplateName " ;
		
			Query query = getSession().createQuery(fromClause);
			if(workflowTemplate.getWorkflowTemplateId() > 0){
				query.setLong("workflowTemplateId", workflowTemplate.getWorkflowTemplateId());
			}
			query.setString("workflowTemplateName", CommonUtils.getEscapedSQLString(workflowTemplate.getWorkflowTemplateName()).toUpperCase());
			
			List list = query.list();

			if (list != null && list.size() > 0) 
				return (WorkflowTemplate) list.get(0);
	...
	@Override
	public List<WorkflowTemplate> findByProperty(String propertyName,
			Object value, long tenancyId) {
		try 
	  	{
		  	log.info("finding WorkflowTemplate instance with property: "+ propertyName + ", value: " + value);
			String queryString = "select workflowTemplate from WorkflowTemplate workflowTemplate where workflowTemplate."+ propertyName + "= ?";
