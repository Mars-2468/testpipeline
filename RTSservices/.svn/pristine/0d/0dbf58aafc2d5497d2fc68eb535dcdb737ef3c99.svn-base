...
<div class="mainHdr">
	<h3 style="height: 30px; font-size: 15px; color: #01293d">
		...
		<fmt:message key="Application form for Property Tax Demand" />
		:
...
<div valign="top" id="SetFormHeight">
	<input type="hidden" name="id"
		value="${propertyTaxAssessment.propertyDemandID}">
	<c:set var="doc"
...
<td colspan="1.5"><span class="ClsLabel" style="font-size: 14px"><fmt:message
		...
		<tr>
			...
			<td colspan="2"><textarea
					...
					maxlength="30" name="address" disabled>${propertyTaxAssessment.nameOfProperyHolder}</textarea></td>
...
<td colspan="6"
										...
										style="height: 37px; padding: 7px 10px 11px; font-size: 17px; color: #01293d; font-style: italic;"><fmt:message
												key="List of Documents (Attachment) For Property Tax Demand" />:</span></td>
			...
			<tr>
				<td colspan="3.5"><span class="ClsLabel"
					style="font-size: 14px"><fmt:message
							...
							Receipt (मालमत्ता कर भरणा पावती)" />:</span> 
				<td style="text-align: center" colspan="3.5"><a class="bg-button btn btn-primary bd-highlight action"
   type="button"
   target="_blank"
...
<%-- <c:if test="${doc[3] != 'null'}">
			<tr>
				<td colspan="3.5"><span class="ClsLabel"
					style="font-size: 14px"><fmt:message
							key="Copy of Electric
								Bill (इलेक्ट्रिक बिलाची प्रत)" />:</span> 
	...
	<input type="hidden" class="ClsTextbox_mr" style="width: 180px"
		...
		value="<c:out value="${propertyTaxAssessment.propertyDemandID}"/>" /> 

			  <input type="hidden" id="rtiApplicationRefId" name="rtiApplicationRefId" value="" />
			  			  <input type="hidden" id="rtiApplicationRefIds" name="rtiApplicationRefIds" value="${propertyTaxAssessment.rtiRefId}" />
