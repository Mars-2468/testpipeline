...
import com.mars.rti.ws.model.TradeLicencePartnerChangeRestDTO;
...
@Controller
public class TradeLicencePartnerChangeController {
...
//	}
	...
	@RequestMapping("/rtsapplication/saveTradeLicencePartnerChange.do")
	// rtsapplication/saveZoneCertificates.do
	public ModelAndView saveTradeLicencePartnerChange(HttpServletRequest request, HttpServletResponse response,
			TradeLicencePartnerChange tradeLicencePartnerChange) throws ServletException, Exception {
		...
		if (mobileNumber != null) {
			...
			if (tradeLicencePartnerChange != null) {
				TradeLicencePartnerChangeRestDTO tradeLicencePartnerChangeRestDTO = new TradeLicencePartnerChangeRestDTO();
				...
				rtiApplication.setPdfUploadFromPortal(tradeLicencePartnerChange.getFilesPath());
				...
				if (savedRti.getRtiApplicationId() > 0) {
					...
					long trade_regis_id = tradeLicencePartnerChangeService.getReferenceId(tradeLicencePartnerChange);
					...
					String mobileNo = tradeLicencePartnerChange.getMobileNo();
					String email = tradeLicencePartnerChange.getEmail();
					SendSMSEmailController.sendApplicationSubmitted(name, mobileNo, applnNo, email);
				}

//				ModelAndView modelAndView = new ModelAndView("newRTITradeRenewalPartnerChangeSuccess");
...
//				return modelAndView;
				...
				if (as_status != null) {
					...
					return new ModelAndView(
							"redirect:/ws/rtsapplication/newRtSTradeLicensePartnerChangeSuccess.do?rtsAppNo="
	...
	@RequestMapping("ws/rtsapplication/newRtSTradeLicensePartnerChangeSuccess.do")
	public ModelAndView newRtSTradeLicensePartnerChangeSuccess(HttpServletRequest request,
			HttpServletResponse response) {

		String appNo = request.getParameter("rtsAppNo");
		ModelAndView model = new ModelAndView("newRTITradeRenewalPartnerChangeSuccess");

		model.addObject("tradeLicencePartnerChange", appNo);
		return model;

	}

	@RequestMapping("/rtiapplication/editTradeLicencePartnerChange.do")
	public ModelAndView editTradeLicenceRenewal(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, Exception {
		log.info("Document Uploaded");
		HttpSession session = request.getSession();
		System.out.println("hiiii edittradeCertificate");
		long rtiApplicationRefId = 0;
		TradeLicencePartnerChange tradeLicencePartnerChange = null;
		long tradeLicencePartnerChangeId = 0;
		...
		if (request.getParameter("rtiApplicationRefId") != null
			...
			if (request.getParameter("rtiApplicationRefId") != null
				...
				tradeLicencePartnerChange = tradeLicencePartnerChangeService.get(rtiApplicationRefId);
			...
			if (tradeLicencePartnerChange.getRti_ref_id() > 0) {
				rtiApplicationId = tradeLicencePartnerChange.getRti_ref_id();
		...
		ModelAndView modelAndView = new ModelAndView("manageTradeLicencePartnerChange", "tradeLicencePartnerChange",
				tradeLicencePartnerChange);
		log.info("success");
		return modelAndView;

	}

	@RequestMapping("/rtiapplication/createTradeLicencePartnerChangeWork.do")
	public ModelAndView createTradeLicenceRenewalWork(HttpServletRequest request, HttpServletResponse response)
		...
		TradeLicencePartnerChange tradeLicencePartnerChange = tradeLicencePartnerChangeService.get(applicationId);
		String email = tradeLicencePartnerChange.getEmail();
		String mobileNo = tradeLicencePartnerChange.getMobileNo();
		...
		return new ModelAndView("redirect:/rtiapplication/editTradeLicencePartnerChange.do");
	}
