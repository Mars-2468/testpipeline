...
public class BankDAOImpl extends HibernateDaoSupport implements BankDAO {
	...
	public Bank get(long id) {
		try {
			...
			return (Bank) getHibernateTemplate().get(Bank.class,new Long(id));
    ...
    public Bank merge(Bank entity) {
    	try {
		    ...
		    return (Bank) getHibernateTemplate().merge(entity);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void delete(long id) {
        try {
        	...
        	Bank entity = (Bank) getHibernateTemplate().load(Bank.class, new Long(id));
	...
	public List<Bank> getAll() {
		try {
			if (log.isDebugEnabled()) {
				log.debug("get all Method is called for Bank");
			}
    ...
    public void deleteAll(List<Bank> listBank)
    {
		try
		{
	    	if (log.isDebugEnabled())
	   		{
				log.debug("deleteAll Method is called for Bank");
	    	}
	    	getHibernateTemplate().deleteAll(listBank);
	    	log.debug("deleted successfuly a list of Bank records from database");
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void saveAll(List<Bank> listBank)
    {
		try
		{
	    	...
	    	getHibernateTemplate().saveOrUpdateAll(listBank);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
	...
	{
		try {
			String fromClause = "select bank from Bank bank";
	...
	public List<Bank> getBankListBySearch(BankSearch searchBank){
		...
		try {
			long pageNumber = searchBank.getCurrentPage();
			String orderBy = searchBank.getOrderBy();
			String sortBy = searchBank.getSortBy();

			String bankName = CommonUtils.getEscapedSQLString(searchBank.getBankName());
			...
			if (bankName != null && bankName.length() > 0) {
				...
				sql.append("upper(bank.bankName) like upper('%" + bankName + "%') ");
			}
	...
	public long getBankCount(){
	    try {
			String fromClause = "select count(*) from Bank bank";
			Query query = getSession().createQuery(fromClause);
	...
	public Long getBankCountBySearch(BankSearch searchBank){
		...
		try {
			...
			StringBuffer sql = new StringBuffer();
			String fromClause = "select count(bank) from Bank bank ";
			List list = null;
			boolean flag = false;
	...
	public List<Bank> findByProperty(String propertyName, Object value) 
    {
		try 
	  	{
		  	log.info("finding Bank instance with property: "+ propertyName + ", value: " + value);
			String queryString = "select bank from Bank bank where bank."+ propertyName + "= ?";
	...
	public Bank checkForDuplicates(Bank bank)
	{
		...
		try {
			String fromClause = "select bank from Bank bank where " ;

			if(bank.getBankId() > 0)		
				fromClause = fromClause +" bankId <> "+bank.getBankId()+" and ";
			
			//Sample Code update accordingly
			//fromClause = fromClause +" bank.{Many-to-one}.{Many-to-one}Id = "+bank.get{Many-to-one}().get{Many-to-one}Id()+" and ";
			fromClause = fromClause +" (upper(bank.bankName) = upper('"+ CommonUtils.getEscapedSQLString(bank.getBankName()) +"')) " ;
