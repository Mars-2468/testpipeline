 ...
 * <p>Title: PlotTypeDaoImpl.java </p>
 ...
 */
@Repository("PlotTypeDAO")
public class PlotTypeDAOImpl extends HibernateDaoSupport implements PlotTypeDAO {

    private static final Log log = LogFactory.getLog(PlotTypeDAOImpl.class);
	...
	public PlotType get(long id) {
		try {
			...
			return (PlotType) getHibernateTemplate().get(PlotType.class,new Long(id));
    ...
    public PlotType merge(PlotType entity) {
    	try {
		    ...
		    return (PlotType) getHibernateTemplate().merge(entity);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void delete(long id) {
        try {
    		if(log.isDebugEnabled()){
        		log.debug("deleting PlotType instance");
        	}
        	PlotType entity = (PlotType) getHibernateTemplate().load(PlotType.class, new Long(id));
	...
	public List<PlotType> getAll() {
		try {
			if (log.isDebugEnabled()) {
				log.debug("get all Method is called for PlotType");
			}
			return getHibernateTemplate().loadAll(PlotType.class);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void deleteAll(List<PlotType> listPlotType)
    {
		try
		{
	    	if (log.isDebugEnabled())
	   		{
				log.debug("deleteAll Method is called for PlotType");
	    	}
	    	getHibernateTemplate().deleteAll(listPlotType);
	    	log.debug("deleted successfuly a list of PlotType records from database");
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void saveAll(List<PlotType> listPlotType)
    {
		try
		{
	    	...
	    	getHibernateTemplate().saveOrUpdateAll(listPlotType);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
	...
	{
		try {
			String fromClause = "select plotType from PlotType plotType";
	...
	public List<PlotType> getPlotTypeListBySearch(PlotTypeSearch searchPlotType){
		if(log.isDebugEnabled()){
			log.debug("getPlotTypeListBySearch Method is called");
		}
		
		try{
			long pageNumber=searchPlotType.getCurrentPage();
			String orderBy=searchPlotType.getOrderBy();
			String sortBy=searchPlotType.getSortBy();
			
			String description=CommonUtils.getEscapedSQLString(searchPlotType.getDescription());
			List<PlotType> list=null;
			...
			if(description!=null && description.length()>0){
				if(!flag){
				    ...
				    sql.append("upper(plotType.description) like upper('%"+ description +"%')");
			}
	...
	public long getPlotTypeCount(){
	    try {
			String fromClause = "select count(*) from PlotType plotType";
			Query query = getSession().createQuery(fromClause);
	...
	public long getPlotTypeCountBySearch(PlotTypeSearch searchPlotType){
		...
		try{
			...
			StringBuffer sql=new StringBuffer();
			String fromClause="select count(plotType) from PlotType plotType";
			List list=null;
			boolean flag=false;
	...
	public List<PlotType> getEntityListBySQLQuery(String strSql, Hashtable<String, NullableType> hstDataTypes)
	{
		try{
			...
			query.addEntity(PlotType.class);
			return query.list();
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	...
	public List<PlotType> findByProperty(String propertyName, Object value) 
    {
		try 
	  	{
		  	log.info("finding PlotType instance with property: "+ propertyName + ", value: " + value);
			String queryString = "select plotType from PlotType plotType where plotType."+ propertyName + "= ?";
	...
	public PlotType checkForDuplicates(PlotType plotType)
	{
		...
		try {
			String fromClause = "select plotType from PlotType plotType where " ;

			if(plotType.getPlotTypeId() > 0)		
				fromClause = fromClause +" plotTypeId <> "+plotType.getPlotTypeId()+" and ";
			
			//Sample Code update accordingly
			//fromClause = fromClause +" plotType.{Many-to-one}.{Many-to-one}Id = "+plotType.get{Many-to-one}().get{Many-to-one}Id()+" and ";
			fromClause = fromClause +" (upper(description) = upper('"+ CommonUtils.getEscapedSQLString(plotType.getDescription()) +"')) " ;
		
			Query query = getSession().createQuery(fromClause);		
			List list = query.list();

			if (list != null && list.size() > 0) 
				return (PlotType) list.get(0);
