...
function isAlphaSpace(parm,pMsg){
	var lwr = 'abcdefghijklmnopqrstuvwxyz';
	var upr = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
	var spChar=' '+'-';
	var val =Trim(document.getElementById(parm).value);
...
/************************** PENDING FUNCTIONS (Need to be removed if not used) **************************/
/*********************************** Generic Functions ***********************************/
/***
...
function addPreviousRowToTable(tableId,fileId,filePath){
	...
	for (var i=1;i<=newRow.cells.length;i++){
		if(newRow.cells[i] && newRow.cells[i].innerText!=newRow.cells[i].innerHTML){
			...
			var ctrObjectSec = newRow.cells[i].childNodes.item(3);
			if(ctrObject == null){
			...
			if(ctrObject != null){
				if(ctrObject.type=='hidden'){
					ctrObject.value="0";
					if(ctrObject.getAttribute("isSequence")=="true")
						ctrObject.value= len-1;
				} else if (ctrObject.type=='text'){
					
					if(ctrObject.name=='fieldLabel'){
						strNewId=ctrObject.id+(len-1);
						ctrObject.setAttribute("id",strNewId)
						newRow.cells[i].childNodes.item(Vfori).value="";
					} 			 

				} else if (ctrObject.type=='select-one'){
					
					if(ctrObject.name=='fieldType'){
						strNewId=ctrObject.id+(len-1);
						ctrObject.setAttribute("id",strNewId)
						newRow.cells[i].childNodes.item(Vfori).value="";
					} 
					if(ctrObject.name=='fieldDataType'){
						strNewId=ctrObject.id+(len-1);
						ctrObject.setAttribute("id",strNewId)
						newRow.cells[i].childNodes.item(Vfori).value="";
					} 
				  ...
				  newRow.cells[i].childNodes.item(Vfori).value="";
				...
				else if (ctrObject.type=='radio'){						
					if(ctrObject.id=='string'){
						strNewId=ctrObject.id+(len-1);
						strNewname=ctrObject.name+(len-1);
						ctrObject.setAttribute("id",strNewId)
						ctrObject.setAttribute("name",strNewname) 
					} 
					...
					newRow.cells[i].childNodes.item(Vfori).value="";
					 if (ctrObjectSec.type == 'radio'){						
						 
							if(ctrObjectSec.id=='number'){
								strNewId=ctrObjectSec.id+(len-1);
								strNewname=ctrObjectSec.name+(len-1);
								ctrObjectSec.setAttribute("id",strNewId)
								ctrObjectSec.setAttribute("name",strNewname) 
							} 
							if(ctrObjectSec.id=='no'){
								strNewId=ctrObjectSec.id+(len-1);
								strNewname=ctrObjectSec.name+(len-1);
								ctrObjectSec.setAttribute("id",strNewId)
								ctrObjectSec.setAttribute("name",strNewname)							 
							}  		
							newRow.cells[i].childNodes.item(3).value="";
					}
				}
...
function addPreviousRowToTableModified(tableId,href,filePath,fileId){
	$('input.ClsTextboxDate').datepicker('destroy');
	table = document.getElementById(tableId);
	...
	for (var i=1;i<=newRow.cells.length;i++)
	{
		if(newRow.cells[i] && newRow.cells[i].innerText!=newRow.cells[i].innerHTML)
		{
			...
			{
					//ctrObject.innerHTML="<b>"+fileId+"</b>";
					newRow.cells[i].innerHTML="<div><b>"+href+"</b>"+
					"<input type='hidden' id='fileName' name='fileName'	value='"+fileId+"' />"+
					"<input type='hidden' id='filePath' name='filePath'	value='"+filePath+"' /></div>";
...
function openSelectTenancy(){
	try{
	
	document.getElementById('managePopup').src=contextRoot+'/tenancy/selectTenancy.do';
	$('#managePopup').attr('height','90px');
	...
	//$('#dialog').dialog( "option", "title", 'Select Department' );
...
{	
	...
	$('#dialog').dialog('close');
	scroll(0,0);
	onPageSubmit(contextRoot+'/workflow/listTasks.do');
