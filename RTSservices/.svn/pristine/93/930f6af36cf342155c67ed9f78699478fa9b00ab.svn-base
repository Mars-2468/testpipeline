...
public class SectionDAOImpl extends HibernateDaoSupport implements SectionDAO {
	...
	public Section get(long id) {
		try {
			...
			return (Section) getHibernateTemplate().get(Section.class,new Long(id));
    ...
    public Section merge(Section entity) {
    	try {
		    ...
		    return (Section) getHibernateTemplate().merge(entity);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void delete(long id) {
        try {
        	...
        	Section entity = (Section) getHibernateTemplate().load(Section.class, new Long(id));
	...
	public List<Section> getAll() {
		try {
			if (log.isDebugEnabled()) {
				log.debug("get all Method is called for Section");
			}
    ...
    public void deleteAll(List<Section> listSection)
    {
		try
		{
	    	if (log.isDebugEnabled())
	   		{
				log.debug("deleteAll Method is called for Section");
	    	}
	    	getHibernateTemplate().deleteAll(listSection);
	    	log.debug("deleted successfuly a list of Section records from database");
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void saveAll(List<Section> listSection)
    {
		try
		{
	    	...
	    	getHibernateTemplate().saveOrUpdateAll(listSection);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
	...
	{
		try {
			String fromClause = "select department from Section department";
	...
	public List<Section> getSectionListBySearch(SectionSearch searchSection){
		...
		if (log.isDebugEnabled()) {
			log.debug("getSectionListBySearch Method is called");
		}

		try {
			long pageNumber = searchSection.getCurrentPage();
			String orderBy = searchSection.getOrderBy();
			String sortBy = searchSection.getSortBy();

			String name = CommonUtils.getEscapedSQLString(searchSection.getName());
			String status = searchSection.getStatus();
	...
	public long getSectionCount(){
	    try {
			String fromClause = "select count(*) from Section department";
			Query query = getSession().createQuery(fromClause);
	...
	public long getSectionCountBySearch(SectionSearch searchSection){
		...
		try {
			...
			//int count = 0;
			StringBuffer sql = new StringBuffer();
			String fromClause = "select count(department) from Section department ";
			List list = null;
			boolean flag = false;
	...
	public List<Section> findByProperty(String propertyName, Object value) 
    {
		try 
	  	{
		  	log.info("finding Section instance with property: "+ propertyName + ", value: " + value);
		  	
			String queryString = "select section from Section section where section."+ propertyName + "= ?";
	...
	public Section checkForDuplicates(Section department)
	{
		...
		try {
			String fromClause = "select department from Section department where " ;

			if(department.getSectionId() > 0)		
				fromClause = fromClause +" departmentId <> "+department.getSectionId()+" and ";
