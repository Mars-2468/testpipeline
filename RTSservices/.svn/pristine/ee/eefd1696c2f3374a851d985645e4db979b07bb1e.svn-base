...
@Controller
public class TradeLicenseCancellationController {

	private static Log log = LogFactory.getLog(TradeLicenseCancellationController.class);
	@Autowired
...
// 	}
	...
	@RequestMapping("/rtsapplication/saveTradeLiCenseCancellation.do")
	public ModelAndView saveTradeLicenseCancellation(HttpServletRequest request, HttpServletResponse response,
			TradeLicenseCancellation tradeLicenseCancellation) throws ServletException, Exception {
		...
		if (log.isDebugEnabled()) {
			log.debug("Invoking tradeLicenseCancellation");
		}
		String mobileNumber = "";
		...
		if (mobileNumber != null) {
			...
			if (tradeLicenseCancellation != null) {
				...
				rtiApplication.setApplicantName(tradeLicenseCancellation.getFirstName() + " "
						+ tradeLicenseCancellation.getMiddleName() + " " + tradeLicenseCancellation.getLastName());
				...
				rtiApplication.setPdfUploadFromPortal(tradeLicenseCancellation.getFilesPath());
				...
				if (savedRti.getRtiApplicationId() > 0) {
					...
					String name = tradeLicenseCancellation.getFullName();
					String applnNo = rti.getRtiApplnNumber();
					String mobileNo = tradeLicenseCancellation.getMobileNumber();
					String email = tradeLicenseCancellation.getEmailID();
					SendSMSEmailController.sendApplicationSubmitted(name, mobileNo, applnNo, email);
				}
//				ModelAndView model = new ModelAndView("newRTITradeLicenseCancellationSuccess",
//						"tradeLicenseCancellation", tradeLicenseCancellation);
//				return model;
				...
				if (as_status != null) {
					...
					return new ModelAndView(
							"redirect:/ws/rtsapplication/newRtSTradeLicenseCancellationSuccess.do?rtsAppNo="
	...
	@RequestMapping("ws/rtsapplication/newRtSTradeLicenseCancellationSuccess.do")
	public ModelAndView newRtSTradeLicenseCancellationSucces(HttpServletRequest request, HttpServletResponse response) {

		String appNo = request.getParameter("rtsAppNo");
		ModelAndView model = new ModelAndView("newRTITradeLicenseCancellationSuccess");

		model.addObject("tradeLicenseCancellation", appNo);
		return model;

	}

	@RequestMapping("/rtiapplication/editTradeLicenseCancellation.do")
	public ModelAndView editTradeLicenseCancellation(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, Exception {

		TradeLicenseCancellation tradeLicenseCancellation = null;
		HttpSession session = request.getSession();
		System.out.println(" editTradeLicenseCancellation");
		...
		if (request.getParameter("rtiApplicationRefId") != null
			...
			if (request.getParameter("rtiApplicationRefId") != null
				...
				tradeLicenseCancellation = tradeLicenseCancellationService.get(rtiApplicationRefId);
			...
			if (tradeLicenseCancellation.getRtiRefId() > 0) {
				rtiApplicationId = tradeLicenseCancellation.getRtiRefId();
		...
		ModelAndView modelAndView = new ModelAndView("manageTradeLicenseCancellation", "tradeLicenseCancellation",
	...
	@RequestMapping("/rtiapplication/createTradeLicenseCancellationWork.do")
	public ModelAndView createTradeLicenseCancellationWork(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, Exception {
		...
		TradeLicenseCancellation tradeLicenseCancellation = tradeLicenseCancellationService.get(applicationId);
		String email = tradeLicenseCancellation.getEmailID();
		String mobileNo = tradeLicenseCancellation.getMobileNumber();
		...
		return new ModelAndView("redirect:/rtiapplication/editTradeLicenseCancellation.do");
	}
