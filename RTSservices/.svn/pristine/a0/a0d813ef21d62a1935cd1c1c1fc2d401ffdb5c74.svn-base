...
<div valign="top" id="SetFormHeight">
	...
	<div id="SearchDivId" class="ClsSearch">
		...
		<table id="SearchTable" class="searchForm" style="display: none;">
			...
			<tr>
				<td colspan="5"><input type="submit" class="ClsButton"
					id="search" name="search"
					value="<fmt:message key="common.button.search"/>"
					onclick="javascript:searchRTIApplication()" /> &nbsp; <input
					type="button" class="ClsButton" id="clear" name="clear"
		...
		<tbody>
			...
			<c:choose>
				<c:when test="${not empty requestScope.rtiapplicationList}">
					<c:forEach var="rtiApplnList"
					...
					</c:if> <c:if test="${rtiApplnList.workFlowStatus==3}">
						...
						Rejected
					</c:if> <c:if test="${rtiApplnList.workFlowStatus==12}">
						Citizen Visit Date Rescheduled
					</c:if></td>
							<input type="hidden" id="rtiApplicationRefId"
								name="rtiApplicationRefId"
								value="<c:out value="${rtiApplnList.rtiApplicationRefId}" />" />
						</tr>
					</c:forEach>
...
<input type="hidden" id="rtiApplicationRefId" name="rtiApplicationRefId" value="<c:out value="${rtiApplnList.rtiApplicationRefId}" />" />
		...
		<tfoot>
			<tr>
				<td colspan="4"><c:if
						test="${requestScope.rtiApplicationList != null || requestScope.rtiapplicationList !=null && requestScope.totalCount>0}">
						<span><%@include file="/pages/common/pagination.jsp"%></span>
					</c:if></td>
			</tr>
		</tfoot>
	</table>
	<input type="hidden" id="rtiApplicationId" name="rtiApplicationId"
		value="" /> <input type="hidden" id="SortBy" name="SortBy"
