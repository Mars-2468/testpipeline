...
import com.mars.rti.controller.PropertyTaxUploadURLGenerationController;
...
import com.mars.rti.ws.model.PropertyTaxReAssessmentRestDTO;

@Controller
public class PropertyTaxReAssessmentRestController {

	private Log log = LogFactory.getLog(PropertyTaxReAssessmentRestController.class);
	...
	@RequestMapping(method = RequestMethod.POST, value = "/savePropertyTaxReAssessment")
	public @ResponseBody PropertyTaxReAssessmentRestDTO savePropertyTaxReAssessment(
			@RequestBody PropertyTaxReAssessmentRestDTO pttr, HttpServletRequest request, HttpServletResponse response)
			throws ServletException {

		PropertyAssessmentTrasScript ptr = new PropertyAssessmentTrasScript();
		...
		try {
			...
			if (pttr.getPropertyId() != null && !pttr.getPropertyId().isEmpty()) {
				...
				String propertyId = "Property ID is Required";
				condition = false;
				message.append(propertyId).append(",");
			}
			...
			files[1] = pttr.getBuildingMapDoc();

			files[2] = pttr.getWaterBIllDoc();

			files[3] = pttr.getAnyOtherSupportingDoc();
			...
			if (condition) {
				Map<String, String> savedFilePath = PropertyTaxReAssessmentRestController.saveFile(files, request);
				...
				if (savedRti.getRtiApplicationId() > 0) {
					...
					long trade_regis_id = propertyAssessmentService.propertyAssessment(ptr);
					...
					if (responseCode.equals("GCPT10100")) {
						if (result != null && !result.isEmpty()) {
							...
							ptr = propertyAssessmentService.merge(ptr);
							...
							pttr.setBuildingMapDoc("");
							pttr.setWaterBIllDoc("");
		...
		if (data != null) {
			for (String element : data) {
				if (element != null) {
					...
					filesPath = url + "PropertyAssessment" + File.separator + fileName;
