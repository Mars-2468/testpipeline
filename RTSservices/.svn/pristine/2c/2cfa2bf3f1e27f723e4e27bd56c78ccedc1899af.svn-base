...
@Controller
public class TradeLicenceRenewalController {
...
//	}
	...
	@RequestMapping("/rtsapplication/saveTradeLicenceRenewal.do")
	// rtsapplication/saveZoneCertificates.do
	public ModelAndView saveTradeLicenceRenewal(HttpServletRequest request, HttpServletResponse response,
			TradeLicenceRenewal tradeLicenceRenewal) throws ServletException, Exception {
		...
		if (mobileNumber != null) {
			...
			if (tradeLicenceRenewal != null) {
				TradeLicenceRenewalRestDTO tradeLicenceRenewalRestDTO = new TradeLicenceRenewalRestDTO();
				RTIApplication rtiApplication = new RTIApplication();

				rtiApplication.setCreatedDate(CommonUtils.getCurrentStringDateAndTime());
				rtiApplication.setRegistrationDate(CommonUtils.getCurrentStringDateAndTime());
				rtiApplication.setTemplateName("marketDepartment");
				rtiApplication.setSubject("TRADE-LICENCE-RENEWAL");
				rtiApplication.setDepartment("MARKET-DEPARTMENT");				rtiApplication.setWorkFlowStatus(0);
				rtiApplication.setFinalStatus("0");
				rtiApplication.setRtiserviceid(33);
				...
				rtiApplication.setPdfUploadFromPortal(tradeLicenceRenewal.getFilesPath());
				...
				if (savedRti.getRtiApplicationId() > 0) {
					...
					long trade_regis_id = tradeLicenceRenewalService.getReferenceId(tradeLicenceRenewal);
...
//						tradeLicenceRenewalRestDTO.setResponse(retunRti.getRtiApplicationId());
//						tradeLicenceRenewalRestDTO.setResult(retunRti.getRtiApplnNumber());
//
//					}
					String name = tradeLicenceRenewal.getFirstName() + tradeLicenceRenewal.getMiddleName()
							+ tradeLicenceRenewal.getLastName();
					String applnNo = rti.getRtiApplnNumber();
					String mobileNo = tradeLicenceRenewal.getMobileNo();
					String email=tradeLicenceRenewal.getEmail();
					SendSMSEmailController.sendApplicationSubmitted(name, mobileNo, applnNo, email);
				}
//				ModelAndView modelAndView = new ModelAndView("newRTITradeRenewalSuccess");
...
//				return modelAndView;
				...
				return new ModelAndView("redirect:/ws/rtsapplication/newRtSTradeLicenseRenewalSuccess.do?rtsAppNo="+retunRti.getRtiApplnNumber());
				}
				}
		}
	...
	@RequestMapping("ws/rtsapplication/newRtSTradeLicenseRenewalSuccess.do")
	public ModelAndView newRtSTreeCuttingSuccess(HttpServletRequest request,HttpServletResponse response) {
		...
		ModelAndView model = new ModelAndView("newRTITradeRenewalSuccess");
		
		model.addObject("tradeLicenceRenewal",appNo);
		return model;
				
		
	}

	@RequestMapping("/rtiapplication/editTradeLicenceRenewal.do")
	public ModelAndView editTradeLicenceRenewal(HttpServletRequest request, HttpServletResponse response)
		...
		TradeLicenceRenewal tradeLicenceRenewal = null;
		long tradeLicenceRenewalid = 0;
		...
		if (request.getParameter("rtiApplicationRefId") != null
			...
			if (request.getParameter("rtiApplicationRefId") != null
				...
				tradeLicenceRenewal = tradeLicenceRenewalService.get(rtiApplicationRefId);
			...
			if (tradeLicenceRenewal.getRti_ref_id() > 0) {
				rtiApplicationId = tradeLicenceRenewal.getRti_ref_id();
		...
		ModelAndView modelAndView = new ModelAndView("manageTradeLicenceRenewal", "tradeLicenceRenewal",
				tradeLicenceRenewal);
		log.info("success");
		return modelAndView;
	...
	@RequestMapping("/rtiapplication/createTradeLicenceRenewalWork.do")
	public ModelAndView createTradeLicenceRenewalWork(HttpServletRequest request, HttpServletResponse response)
		...
		TradeLicenceRenewal tradeLicenceRenewal = tradeLicenceRenewalService.get(applicationId);
		String email = tradeLicenceRenewal.getEmail();
		String mobileNo = tradeLicenceRenewal.getMobileNo();
		...
		return new ModelAndView("redirect:/rtiapplication/editTradeLicenceRenewal.do");
	}
