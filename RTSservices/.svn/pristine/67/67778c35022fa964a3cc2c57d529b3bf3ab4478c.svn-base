...
function editDesignation(id)
{
	...
	onPageSubmit('<c:out value="${contextRoot}"/>/designation/editDesignation.do');
}
function searchDesignation(){
	onPageSubmit('<c:out value="${contextRoot}"/>/designation/listDesignation.do');
}
</script>
<div class="mainHdr">
	<h3> <!-- <a href="<c:out value="${contextRoot}"/>/designation/editDesignation.do">
			<fmt:message key="designation.manage.pagetitle.new"/> 
		</a> -->
		<fmt:message key="designation.list.pagetitle.list"/>
	</h3>
</div>
...
<div id="SearchDivId" class="ClsSearch">
		...
		<table id="SearchTable" class="searchForm" style="display:none;">
			<tr>
				<td>
					<fmt:message key="designation.list.label.name" />
				</td>
				<td>
					<input type="text" class="ClsTextbox" id="searchName" maxlength="100" size="50" 
						style="width:180px" name="searchName" 
						value="<c:out value="${SearchDesignation.name}"/>" />
					&nbsp;
				</td>
				<td colspan="2" align="right">
					<input type="submit" class="ClsButton" id="search" name="search" value="<fmt:message key="common.button.search"/>" onclick="javascript:searchDesignation()" />
					&nbsp;
					<input type="button" class="ClsButton" id="clear" name="clear" value="<fmt:message key="common.button.clear"/>" onclick="javascript:clearFields('SearchTable');searchDesignation()" />
				</td>
			</tr>
		</table>
		<c:if test="${SearchDesignation.searchParamSet == true}">
			<script>
	...
	<table cellpadding="0" cellspacing="1" width="98%" class="dataGrid" >
		...
		<tbody>
			...
			<c:forEach var="DesignationList" items="${requestScope.DesignationList}" varStatus="rowNumber">
			<c:set var="hasRows" value="true" />
			<tr>
				<td><a title="<fmt:message key="list.link.EditOrViewLink"/>" href="javascript:editDesignation('${DesignationList.designationId}');">
				<c:out value="${DesignationList.name}"/></a></td>
			</tr>
			</c:forEach>
		...
		<tfoot>
			<tr>
				<td colspan="2">
					<c:if test="${requestScope.DesignationList != null}">
...
<input type="hidden" id="designationId" name="designationId" value="<c:out value="${designation.designationId}"/>" />
