...
@Repository("renewalNursingHomesDAO")
public class RenewalNursingHomesDAOImpl extends HibernateDaoSupport implements RenewalNursingHomesDAO {
	
	

	private static final Log log =LogFactory.getLog(RenewalNursingHomesDAOImpl.class);
	...
	public RenewalNursingHomes findRenewalNurshingCertificateIdFromRefId(long id) {
		...
		String HQL="select zc from RenewalNursingHomes d where d.rtiapplrefno="+id;
		
		RenewalNursingHomes zc=null;
		
		String sqlquery ="select renewalNursingHomes from RenewalNursingHomes renewalNursingHomes where renewalNursingHomes.rtiApplicationRefId ="+id;
				List<RenewalNursingHomes> list=null;
	...
	public RenewalNursingHomes get(long rtiApplicationRefId) {
		try {
			...
			return (RenewalNursingHomes) getHibernateTemplate().get(RenewalNursingHomes.class, new Long(rtiApplicationRefId));
	...
	@Override
	public RenewalNursingHomes saveRenewalNursingHome(RenewalNursingHomes renewalNursingHomes) {
		try
		{
		    if (log.isDebugEnabled())
		    {
		    ...
		    getHibernateTemplate().saveOrUpdate(renewalNursingHomes);
		...
		return renewalNursingHomes;
		
	}

	@Override
	public List<RenewalNursingHomes> getList(
		...
		try {
			...
			String sqlString = "select renewalNursingHomes from RenewalNursingHomes renewalNursingHomes";
			...
			List<RenewalNursingHomes> list = query.list();
	...
	@Override
	public RenewalNursingHomes findFromRTIRefNum(String rtiapplrefno) {
		RenewalNursingHomes d = null;

		String sqlquery = "select renewalNursingHomes from renewalNursingHomes renewalNursingHomes where renewalNursingHomes.rtiapplrefno ="
				+ "'" + rtiapplrefno + "'";
		List<RenewalNursingHomes> list = null;
		Query query = getSessionFactory().getCurrentSession().createQuery(
				sqlquery);
	...
	public RenewalNursingHomes merge(RenewalNursingHomes renewalNursingHomes) {
		...
		try {
			...
			return (RenewalNursingHomes) getHibernateTemplate().merge(renewalNursingHomes);
		} catch (Exception exception) {
			log.error(exception.getStackTrace(), exception);
			exception.printStackTrace();
		}
	...
	public long nursingHomeRenewalApplications(RenewalNursingHomes renewalNursingHomes) {
		RenewalNursingHomes b = null;
		try {
			if (log.isDebugEnabled()) {
				log.debug("save Method is called ");
			}
			b = (RenewalNursingHomes) getHibernateTemplate().merge(renewalNursingHomes);

			if (b != null) {
				return b.getRenewalNursingHomesregId();
			}
		} catch (NullPointerException nullpointerexception) {
			log.error(nullpointerexception.getStackTrace());
			throw new NullPointerException();
