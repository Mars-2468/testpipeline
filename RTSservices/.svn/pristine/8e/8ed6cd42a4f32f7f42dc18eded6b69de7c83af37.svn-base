 ...
 */
@Controller
public class RTIApplicationParkController extends MultiActionController implements InitializingBean
{
    ...
    private static Log log = LogFactory.getLog(RTIApplicationParkController.class);
    ...
    public void clearSessionAttributes(HttpSession session){
		...
		session.setAttribute("park",""); 
    }
    
    
    
    @RequestMapping("/rtiapplication/editRTIApplicationPark.do")
	public ModelAndView editRTIApplicationPark(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, Exception{
		if (log.isDebugEnabled())
		{
		...
		if (org.apache.commons.lang.StringUtils.isNotEmpty(rtiApplicationId) && !rtiApplicationId.equals("0"))
			...
			rtiApplication = rtiApplicationService.get(Long.parseLong(rtiApplicationId));	     
				if(rtiApplication != null ){ 
					...
					return new ModelAndView("rtiApplicationPark", "rtiApplication", rtiApplication);
			} else{
    ...
    @RequestMapping("/rtiapplication/parkRTIApplication.do")
    public ModelAndView parkRTIApplication(HttpServletRequest request, HttpServletResponse response)  throws ServletException,Exception 
    {
    	HttpSession session = request.getSession();
    	SessionUser sessionUser = (SessionUser) session.getAttribute("SessionUser");
		...
		if (log.isDebugEnabled())
		{
			log.debug("Invoking parkRTIApplication");
		}

		RTIApplication rtiApplication=null;			
		String rtiApplicationId = request.getParameter("rtiApplicationId"); 
		request.setAttribute("rtiApplicationId", rtiApplicationId);		
		String parkedRemarks = request.getParameter("parkedRemarks");
		...
		if (StringUtils.isNotEmpty(rtiApplicationId)   && !rtiApplicationId.equals("0") ) {		
			...
			if(rtiApplication != null){	
				if(user!=null && userId>0){	
					List<RTIApplicationPark> rtiApplicationParkList = new ArrayList<RTIApplicationPark>();
					RTIApplicationPark rtiApplicationPark = new RTIApplicationPark(); 					
					rtiApplicationPark.setRtiApplication(rtiApplication);
					rtiApplicationPark.setParkedRemarks(parkedRemarks);
					rtiApplicationPark.setReminderDate(reminderDate);
					rtiApplicationPark.setStatus(1);			
					rtiApplicationPark.setParkedBy(user);	
					rtiApplicationParkList.add(rtiApplicationPark);
										
					rtiApplication.setRtiApplicationPark(rtiApplicationParkList);
					rtiApplication.setIsApplicationParked(1);
					rtiApplication.setFinalStatusWhenParked(rtiApplication.getFinalStatus());
					rtiApplication.setFinalStatus(CoreConstants.APPL_STATUS_PARKED);
					
					
					rtiApplicationService.merge(rtiApplication);
					session.setAttribute("message", "RTI Application Parked Successfully..");
				}			
				else{
		...
		//return getAllUserTask(request, response);
		return editRTIApplication(request, response);
    }

    @RequestMapping("/rtiapplication/activateParkedRTIApplication.do")
    public ModelAndView activateParkedRTIApplication(HttpServletRequest request, HttpServletResponse response)  throws ServletException,Exception 
    {
    	HttpSession session = request.getSession();
    	SessionUser sessionUser = (SessionUser) session.getAttribute("SessionUser");
		long userId = sessionUser.getUserId();
		...
		if (log.isDebugEnabled())
		{
			log.debug("Invoking activateParkedRTIApplication");
		}

		RTIApplication rtiApplication=null;			
		String rtiApplicationId = request.getParameter("rtiApplicationId"); 
		String unParkedRemarks = request.getParameter("unParkedRemarks");
		request.setAttribute("rtiApplicationId", rtiApplicationId);			
		
		if (StringUtils.isNotEmpty(rtiApplicationId)   && !rtiApplicationId.equals("0") ) {		
			rtiApplication = rtiApplicationService.get(Long.parseLong(rtiApplicationId));
			RTIApplicationPark  rtiApplicationPark =rtiApplicationService.getRTIApplicationParkByRTIApplicationIdandStatus(Long.parseLong(rtiApplicationId), 1);
			if(rtiApplication != null && rtiApplicationPark !=null){	
				if(user!=null && userId>0){					
					List<RTIApplicationPark> rtiApplicationParkList = new ArrayList<RTIApplicationPark>();	
					rtiApplicationPark.setRtiApplication(rtiApplication);
					rtiApplicationPark.setUnParkedRemarks(unParkedRemarks);					
					rtiApplicationPark.setStatus(0);			
					rtiApplicationPark.setUnParkedBy(user);	
					rtiApplicationPark.setUnParkedDate(CommonUtils.getCurrentStringDateAndTime());
					rtiApplicationParkList.add(rtiApplicationPark);
					
					rtiApplication.setRtiApplicationPark(rtiApplicationParkList);					
					rtiApplication.setIsApplicationParked(0); 					 
					rtiApplication.setFinalStatus(rtiApplication.getFinalStatusWhenParked());
					rtiApplication.setFinalStatusWhenParked(CoreConstants.APPL_STATUS_UN_PARKED);
					rtiApplicationService.merge(rtiApplication);					
					session.setAttribute("message", "Parked RTI Application Activated Successfully...");
				}			
				else{
					session.setAttribute("errMessage",	"Please Try Again...");
				}				
