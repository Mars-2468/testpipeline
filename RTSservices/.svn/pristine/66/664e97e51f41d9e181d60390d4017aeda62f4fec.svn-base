...
@Controller
public class TradeLicenseTypeChangeController {

	private static Log log = LogFactory.getLog(TradeLicenseTypeChangeController.class);
	@Autowired
...
// 	}
	...
	@RequestMapping("/rtsapplication/saveTradeLicenseTypeChange.do")
	public ModelAndView saveTradeLicenseTypeChange(HttpServletRequest request, HttpServletResponse response,
			TradeLicenseTypeChange tradeLicenseTypeChange) throws ServletException, Exception {
		...
		if (log.isDebugEnabled()) {
			log.debug("Invoking tradeLicenseTypeChange");
		}
		String mobileNumber = "";
		...
		if (mobileNumber != null) {
			...
			if (tradeLicenseTypeChange != null) {
				...
				rtiApplication.setApplicantName(tradeLicenseTypeChange.getFirstName() + " "
						+ tradeLicenseTypeChange.getMiddleName() + " " + tradeLicenseTypeChange.getLastName());
				...
				rtiApplication.setPdfUploadFromPortal(tradeLicenseTypeChange.getFilesPath());
				...
				if (savedRti.getRtiApplicationId() > 0) {
					...
					long birth_regis_id = tradeLicenseTypeChangeService.tradeLicenseTypeChange(tradeLicenseTypeChange);
					...
					String name = tradeLicenseTypeChange.getFullName();
					String applnNo = rti.getRtiApplnNumber();
					String mobileNo = tradeLicenseTypeChange.getMobileNumber();
					String email=tradeLicenseTypeChange.getEmailID();
					SendSMSEmailController.sendApplicationSubmitted(name, mobileNo, applnNo, email);
				}
//				ModelAndView model = new ModelAndView("newRTITradeLicenseTypeChangeSuccess", "tradeLicenseTypeChange",
//						tradeLicenseTypeChange);
//				return model;
				...
				return new ModelAndView("redirect:/ws/rtsapplication/newRtSTradeLicenseTypeChangeSuccess.do?rtsAppNo="+retunRti.getRtiApplnNumber());
				}
	...
	@RequestMapping("ws/rtsapplication/newRtSTradeLicenseTypeChangeSuccess.do")
	public ModelAndView newRtSTradeLicenseTypeChangeSuccess(HttpServletRequest request,HttpServletResponse response) {
		
		String appNo = request.getParameter("rtsAppNo");
		ModelAndView model = new ModelAndView("newRTITradeLicenseTypeChangeSuccess");
		
		model.addObject("tradeLicenseTypeChange",appNo);
		return model;
				
		
	}

	@RequestMapping("/rtiapplication/editTradeLicenseTypeChange.do")
	public ModelAndView editTradeLicenseTypeChange(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, Exception {

		TradeLicenseTypeChange tradeLicenseTypeChange = null;
		HttpSession session = request.getSession();
		System.out.println(" edittradeLicenseTypeChange");
		...
		if (request.getParameter("rtiApplicationRefId") != null
			...
			if (request.getParameter("rtiApplicationRefId") != null
				...
				tradeLicenseTypeChange = tradeLicenseTypeChangeService.get(rtiApplicationRefId);
			} else if (session.getAttribute("rtirefId") != null) {
				Object a = session.getAttribute("rtirefId");
				rtiApplicationRefId = (long) a;
				tradeLicenseTypeChange = tradeLicenseTypeChangeService.get(rtiApplicationRefId);
			}
			if (tradeLicenseTypeChange.getRtiRefId() > 0) {
				rtiApplicationId = tradeLicenseTypeChange.getRtiRefId();
		...
		ModelAndView modelAndView = new ModelAndView("manageTradeLicenseTypeChange", "tradeLicenseTypeChange",
				tradeLicenseTypeChange);
	...
	@RequestMapping("/rtiapplication/createTradeLicenseTypeChangeWork.do")
	public ModelAndView createTradeLicenseTypeChangeWork(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, Exception {
		...
		TradeLicenseTypeChange tradeLicenseTypeChange = tradeLicenseTypeChangeService.get(applicationId);
		String email = tradeLicenseTypeChange.getEmailID();
		String mobileNo = tradeLicenseTypeChange.getMobileNumber();
		...
		return new ModelAndView("redirect:/rtiapplication/editTradeLicenseTypeChange.do");
	}
