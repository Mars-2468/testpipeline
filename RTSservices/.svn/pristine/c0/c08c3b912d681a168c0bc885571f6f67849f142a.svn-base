 ...
 * <p>Title: DistrictDaoImpl.java </p>
...
public class DistrictDAOImpl extends HibernateDaoSupport implements DistrictDAO {
	...
	public District get(long id) {
		try {
			...
			return (District) getHibernateTemplate().get(District.class,new Long(id));
    ...
    public District merge(District entity) {
    	try {
		    ...
		    return (District) getHibernateTemplate().merge(entity);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void delete(long id) {
        try {
        	...
        	District entity = (District) getHibernateTemplate().load(District.class, new Long(id));
	...
	public List<District> getAll() {
		try {
			if (log.isDebugEnabled()) {
				log.debug("get all Method is called for District");
			}
    ...
    public void deleteAll(List<District> listDistrict)
    {
		try
		{
	    	if (log.isDebugEnabled())
	   		{
				log.debug("deleteAll Method is called for District");
	    	}
	    	getHibernateTemplate().deleteAll(listDistrict);
	    	log.debug("deleted successfuly a list of District records from database");
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void saveAll(List<District> listDistrict)
    {
		try
		{
	    	...
	    	getHibernateTemplate().saveOrUpdateAll(listDistrict);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
	...
	{
		try {
			String fromClause = "select district from District district";
	...
	public List<District> getDistrictListBySearch(DistrictSearch searchDistrict){
		if(log.isDebugEnabled()){
			log.debug("getDistrictListBySearch Method is called");
		}
		
		try{
			long pageNumber=searchDistrict.getCurrentPage();
			String orderBy=searchDistrict.getOrderBy();
			String sortBy=searchDistrict.getSortBy();
			
			String name=CommonUtils.getEscapedSQLString(searchDistrict.getName());
			long stateId=searchDistrict.getStateId();
			...
			if((name!=null && name.length()>0)){
				if(!flag){
				    ...
				    sql.append("upper(district.name) like upper('%"+ name +"%')");
			}
			...
			if(stateId>0){
				if(!flag){
				   ...
				   sql.append("district.state.stateId=" +stateId);
			}
	...
	public long getDistrictCount(){
	    try {
			String fromClause = "select count(*) from District district";
			Query query = getSession().createQuery(fromClause);
    ...
    public long getDistrictCountBySearch(DistrictSearch searchDistrict){
    	if(log.isDebugEnabled()){
    		log.debug("getDistrictCountBySearch Method is called");
    	}
    	...
    	try{
    		...
    		StringBuffer sql=new StringBuffer();
    		String fromClause="select count(district) from District district";
    		List list=null;
    		boolean flag=false;
	...
	public List<District> findByProperty(String propertyName, Object value) 
    {
		try 
	  	{
		  	log.info("finding District instance with property: "+ propertyName + ", value: " + value);
			String queryString = "select district from District district where district."+ propertyName + "= ?";
	...
	public District checkForDuplicates(District district)
	{
		...
		try {
			String fromClause = "select district from District district where " ;

			if(district.getDistrictId() > 0)		
				fromClause = fromClause +" districtId <> "+district.getDistrictId()+" and ";
			
			//Sample Code update accordingly
			//fromClause = fromClause +" district.{Many-to-one}.{Many-to-one}Id = "+district.get{Many-to-one}().get{Many-to-one}Id()+" and ";
			fromClause = fromClause +" (upper(name) = upper('"+ CommonUtils.getEscapedSQLString(district.getName()) +"')) and " ;
			fromClause = fromClause +" state.stateId = "+ district.getState().getStateId();
