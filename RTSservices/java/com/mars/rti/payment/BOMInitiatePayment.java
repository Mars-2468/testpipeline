...
public class BOMInitiatePayment {

	private static Log log = LogFactory.getLog(BOMInitiatePayment.class);
	...
	public static String initiatePayment(HashMap<String, String> params) {
		...
		try {                      
			String hashString = "";
			String hash = "";     
			params.put("key", PaymentConstants.BOM_TEST_KEY);
			...
			if (!empty(params.get("key"))) {
				...
				hashString = hashString.concat(PaymentConstants.BOM_TEST_SALT);
	...
	public static boolean compareHash(HttpServletRequest request) {
		...
		params.put("salt", PaymentConstants.BOM_TEST_SALT);
