...
import com.mars.rti.ws.model.DeathCertificateRestTempDTO;

@Controller
public class DeathCertificateRestTempController {

	private static Log log = LogFactory.getLog(DeathCertificateRestTempController.class);
	...
	@RequestMapping(method = RequestMethod.POST, value = "/saveDeathApplicationTemp")
	public @ResponseBody DeathCertificateRestTempDTO saveDeathApplication(
			@RequestBody DeathCertificateRestTempDTO deathCertificateRestDTO, HttpServletRequest request,
...
//			}
			...
			files[5] = doc5;
			String savedFilePath = DeathCertificateRestTempController.saveFile(files, request);
			log.debug("savedFilePath : " + savedFilePath);
	...
	public static String saveFile(String data[], HttpServletRequest request) throws IOException {
		...
		if (data != null) {
			for (String element : data) {
				if (element != null) {
					//byte[] pdfBytes = Base64.getDecoder().decode(element);//Working corerct one Base 64 Code converter
					byte[] pdfBytes;
					try {
					    pdfBytes = Base64.getDecoder().decode(element.replaceAll("\\s+", ""));
					} catch (IllegalArgumentException e) {
					    // Ignore invalid or non-base64 content
