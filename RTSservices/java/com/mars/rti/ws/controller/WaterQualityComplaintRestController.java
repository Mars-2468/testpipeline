...
import com.mars.rti.ws.model.WaterQualityComplaintRestDTO;

@Controller
public class WaterQualityComplaintRestController {

	private static Log log = LogFactory
			.getLog(WaterQualityComplaintRestController.class);
	...
	@RequestMapping(method = RequestMethod.POST, value = "/saveWaterQualityComplaint")
	public @ResponseBody WaterQualityComplaintRestDTO saveWaterQualityComplaint(
			@RequestBody WaterQualityComplaintRestDTO waterdto,
		...
		WaterQualityComplaint water = new WaterQualityComplaint();
		try {
			if (waterdto.getTitle() != null && !waterdto.getTitle().isEmpty()) {
				water.setTitle(waterdto.getTitle());
			} else {

				String title = "Titile field is Required";
				condition = false;
			...
			water.setAadhaarNo(waterdto.getAadhaarNo());
			...
			if (waterdto.getEmail() != null && !waterdto.getEmail().isEmpty()) {
				water.setEmail(waterdto.getEmail());
			} else {
			...
			if (waterdto.getFullAddress() != null
				...
				water.setFullAddress(waterdto.getFullAddress());
			} else {
			...
			if (waterdto.getConsumerAddress() != null && !waterdto.getConsumerAddress().isEmpty()) {
				water.setExitingAddress(waterdto.getConsumerAddress());
			} else {
			...
			if (waterdto.getConsumerNo() != null && !waterdto.getConsumerNo().isEmpty()) {
				water.setConsumerNo(waterdto.getConsumerNo());
			} else {

				String cosumerNo = "Consumer Number field is Required";
				condition = false;
				message.append(cosumerNo).append(",");
			...
			if (waterdto.getMeterNo() != null && !waterdto.getMeterNo().isEmpty()) {
				...
				water.setDreceiptNo(waterdto.getDepositReceiptNo());
			     water.setReceiptDate(waterdto.getReceiptDate());
			...
			if (waterdto.getProblemDescription() != null && !waterdto.getProblemDescription().isEmpty()) {
				water.setProblem(waterdto.getProblemDescription());
			} else {
			...
			//save files method
			String savedFilePath = WaterQualityComplaintRestController
			...
			rtiApplication.setEmail(water.getEmail());
			...
			if (savedRti.getRtiApplicationId() > 0) {
				...
				if ((anonymous_status.equals(a))) {
					...
					String problem = water.getProblem();
				...
				long birth_regis_id = waterQualityComplaintService.waterQualityComplaint(water);
				...
				SendEmail.sendEmail(water.getEmail(),
		...
		waterdto.setResponseStatus("Requested data not savedsuccessfully");
		waterdto.setApplicantIDProof("");
		waterdto.setCopyOfCurrentBill("");
		waterdto.setCopyOfDemandPaidReceipt("");
		waterdto.setCopyOfOnwershipproof("");
	...
	public static String saveFile(String data[], HttpServletRequest request) throws IOException {
		...
		String baseDir = CoreConstants.UPLOAD_PATH + "WaterQualityComplaint";
