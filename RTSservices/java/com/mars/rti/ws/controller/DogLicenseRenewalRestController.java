...
import com.mars.rti.ws.model.DogLicenseRenewalRestDTO;

@Controller
public class DogLicenseRenewalRestController {
	...
	@RequestMapping(method=RequestMethod.GET,value="/getDogDetails")
	public @ResponseBody DogLicenseRenewalRestDTO getDogDetails(@RequestParam String rtsappNo,HttpServletRequest request, HttpServletResponse response)
	        throws ServletException, IOException {
	    DogLicenseCertificate dogLicenseCertificate = null;
	    List<DogDetails> dogDetailsList = new ArrayList<>();
	    DogLicenseRenewalRestDTO dlrrt = new DogLicenseRenewalRestDTO();
	    DateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
		Date date = new Date();
		   List<DogDetailsRestDTO> dddtolist= new ArrayList<>();
	    try {
	    	
	    	if(rtsappNo!=null && !rtsappNo.isEmpty()) {
	    		
	    		
	        RTIApplication rts = rtiApplicationService.findByRTIApplicationNumber(rtsappNo);
	        ...
	        if (dogLicenseCertificate != null && dogDetailsList != null) {
		        long time = checkDogRenewalTime(dateFormat.format(date), dogLicenseCertificate.getCertificate_issued(), dateFormat);
	        	if (time >= 365) {
	        		for(DogDetails dd:dogDetailsList) {
	        			DogDetailsRestDTO dddto= new DogDetailsRestDTO();
		        		BeanUtils.copyProperties(dddto,dd);
		        		//dddto.setDogImage(dd.getDogImagefiles());
		        		dddtolist.add(dddto);
		        		
		        	}
	        		BeanUtils.copyProperties(dlrrt,dogLicenseCertificate);
	        		dlrrt.setDogDetails(dddtolist);
	        		dlrrt.setName(dogLicenseCertificate.getFirstName());
	        		dlrrt.setFatherName(dogLicenseCertificate.getMiddleName());
	        		dlrrt.setSurname(dogLicenseCertificate.getLastName());
	        		dlrrt.setRenewaldogId(dogLicenseCertificate.getRenewaldog());
	        		dlrrt.setFullName(dogLicenseCertificate.getName());
	        		dlrrt.setCityName(dogLicenseCertificate.getApplicantcity());
	        		dlrrt.setResponseStatus("Record Found");
		        	response.setStatus(HttpStatus.OK.value()) ;
		        	dlrrt.setResponseCode(HttpStatus.OK.value());
					dlrrt.setStatus("ok");
					dlrrt.setResult(rtsappNo);
	        		
	        	}else {
	        		
	        		dlrrt.setResponseStatus("Dog license renewal is not applicable at this time. Please renew after the completion of the 1-year license period.");
		        	response.setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value()) ;
		        	dlrrt.setResponseCode(
							HttpStatus.INTERNAL_SERVER_ERROR.value());
					dlrrt.setStatus("Internal Server Error");
					dlrrt.setResult(rtsappNo);

	        }
	        }else {
	        	
	        	dlrrt.setResponseStatus("Record not found. Please enter a valid application number.");
	        	response.setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value()) ;
	        	dlrrt.setResponseCode(
						HttpStatus.INTERNAL_SERVER_ERROR.value());
				dlrrt.setStatus("Internal Server Error");
	    		...
	    		dlrrt.setResponseStatus("Please enter a valid application number.");
	        	response.setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value()) ;
	        	dlrrt.setResponseCode(
						HttpStatus.INTERNAL_SERVER_ERROR.value());
				dlrrt.setStatus("Internal Server Error");
   ...
   public long checkDogRenewalTime(String date2, String date1, DateFormat dateFormat) {
	...
	@RequestMapping(method = RequestMethod.POST, value = "/saveDogLicenseRenewal")
	public @ResponseBody DogLicenseRenewalRestDTO saveDogLicenseRenewal(
			@RequestBody DogLicenseRenewalRestDTO dogdto,
		...
		try {
			...
			if(dogdto.getRenewaldogId()!=null && !dogdto.getRenewaldogId().isEmpty()) {
				dog.setRenewaldog(dogdto.getRenewaldogId());
			}else {
				String renewal = "Existing Dog License No. field is required";
				message.append(renewal).append(",");
				condition=false;
			}
			...
			if(dogdto.getRenewaldogIdProof()!=null && !dogdto.getRenewaldogIdProof().isEmpty()) {
				files[4]=dogdto.getRenewaldogIdProof();
			}else {
				String rewenalIdproof="Existing License  ID proof";
				condition=false;
				message.append(rewenalIdproof).append(",");
			}
			...
			if (dogdto.getDogDetails().get(0).getGender() == null
				...
				if (dogdto.getDogDetails().get(0).getDogType().equals("IndianDog")
						&& dogdto.getDogDetails().get(0).getOperated().equals("Operated")) {
