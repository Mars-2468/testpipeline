...
import com.mars.rti.ws.model.WaterMeterDTO;

@Controller
public class WaterMeterController {

	private static Log log = LogFactory
			.getLog(WaterMeterController.class);
	...
	@RequestMapping(method = RequestMethod.POST, value = "/saveWaterConnectionComplaintMeter")
	public @ResponseBody WaterMeterDTO saveWaterConnectionComplaintMeter(
			@RequestBody WaterMeterDTO waterdto,
		...
		WaterMeterComplaint water = new WaterMeterComplaint();
		try {
			...
			if (waterdto.getApplicantcity() != null
				...
				water.setFullAddress(waterdto.getApplicantcity());
			} else {
			...
			if (waterdto.getConsumerAddress() != null && !waterdto.getConsumerAddress().isEmpty()) {
				water.setConsumer(waterdto.getConsumerAddress());
			} else {
			...
			if (waterdto.getProblemDescription() != null && !waterdto.getProblemDescription().isEmpty()) {
				water.setProblemDesc(waterdto.getProblemDescription());
			} else {
			...
			//save files method
			String savedFilePath = WaterMeterController
			...
			rtiApplication.setSubject("METER-WATER-CONNECTION");
			...
			if (savedRti.getRtiApplicationId() > 0) {
				...
				if ((anonymous_status.equals(a))) {
					...
					String problem = water.getProblemDesc();
					...
					formData.add("Comp_Subcat1", "meter_water");
					formData.add("Comp_Subcat2", "meter_water");
				...
				long birth_regis_id = waterMeterComplaintService.waterMeterComplaint(water);
	...
	public static String saveFile(String data[], HttpServletRequest request) throws IOException {
		...
		String baseDir = CoreConstants.UPLOAD_PATH + "waterMeterComplaint";
		...
		if (data != null) {
			for (String element : data) {
				if (element != null) {
					...
					filesPath = url + "waterMeterComplaint" + File.separator + fileName;
