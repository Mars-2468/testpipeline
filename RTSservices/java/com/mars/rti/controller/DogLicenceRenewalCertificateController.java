...
@Controller
public class DogLicenceRenewalCertificateController {
	@Autowired
	...
	@RequestMapping("/rtiapplication/saveDogRenewalLicenceCertificate.do")
	public ModelAndView saveDogRenewalLicenceCertificate(HttpServletRequest request, HttpServletResponse response,
...
//			User user=userService.get(sessionUser.getUserId());
		...
		if (mobileNumber != null) {
			...
			if (dogLicenseCertificate != null) {
				...
				rtiApplication.setSubject("DOGLICENCE-RENEWAL-CERTIFICATE");
				rtiApplication.setDepartment("SOLIDWASTE-DEPARTMENT");
				rtiApplication.setWorkFlowStatus(0);
				rtiApplication.setFinalStatus("0");
				rtiApplication.setRtiserviceid(102);
				...
				rtiApplication.setApplicationCost(total);
				...
				RTIApplication savedRti = rtiApplicationService.merge(rtiApplication);
				 ...
				 if(dogLicenseCertificate.getRenewaldog()!=null && !dogLicenseCertificate.getRenewaldog().isEmpty()) {
					   
					 dogLicenseCertificate.setRenewalStatus(1);
				 }
...
//				return modelAndView;
				...
				return new ModelAndView("redirect:/ws/rtsapplication/newRtSDogRenewalLicenseSuccess.do?rtsAppNo="+retunRti.getRtiApplnNumber());
				}
	...
	@RequestMapping("ws/rtsapplication/newRtSDogRenewalLicenseSuccess.do")
	public ModelAndView newRtSDogRenewalLicenseSuccess(HttpServletRequest request,HttpServletResponse response) {
	...
	@RequestMapping("/rtsapplication/editDogRenewalLicenseCertificate.do")
	public ModelAndView editDogRenewalLicenseCertificate(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, Exception {
		HttpSession session = request.getSession();
		System.out.println("hiiii editDogRenewalLicenseCertificate");
...
//				 request.setAttribute("zonelist",newzonelist);
		...
		ModelAndView modelAndView = new ModelAndView("manageDogRenewalLicense");
...
//	   	   modelAndView.addObject("dogDetailsList", dogDetailsList);
	...
	@RequestMapping("/rtiapplication/createDogRenewalLicenceCertificateWork.do")
	public ModelAndView createDogRenewalLicenceCertificateWork(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, Exception {
			...
			if (workflowProcessDetails != null) {
				...
				if (!workflowService.isProcessEnded(processId)) {
					...
					if(dogLicenseCertificate.getRenewaldog()!=null && !dogLicenseCertificate.getRenewaldog().isEmpty()) {
						 ...
						 dogLicenseCertificate.setRenewalStatus(0);
					 }
		...
		return new ModelAndView("redirect:/rtsapplication/editDogRenewalLicenseCertificate.do");
	}

	

//
	@RequestMapping(method = RequestMethod.POST, value = "/rtsapplication/getDogApplicationsDetails.do")
	public void getDogApplicationsDetails(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			renewalDog = (renewalDog == null) ? "" : renewalDog.trim();

			  if (renewalDog != null && renewalDog.startsWith("RTS/")) {

		            // RTS/SWM/3933/2024
		            rts = rtiApplicationService.findByRTIApplicationNumber(renewalDog);

		        } else if (renewalDog != null) {

		            // 87/SWM/New
		            String dogIdStr = renewalDog.split("/")[0];
		            dogId = Long.parseLong(dogIdStr);
		            dogDetails = dogLicenceCertificateService.getDogDetails(dogId);
		        }
			  ...
			  if (rts != null) {
		            ...
		            dogLicenseCertificate =
		                    dogLicenceCertificateService.get(rts.getRtiApplicationRefId());

		        } else if (dogDetails != null && dogDetails.getDogLicenseCertificate() != null) {

		        	Long id= dogDetails.getDogLicenseCertificate().getDogId();
		            dogLicenseCertificate =
		                    dogLicenceCertificateService.get(
		                            dogDetails.getDogLicenseCertificate().getDogId()
		                    );
		        }
