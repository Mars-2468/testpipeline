...
import com.mars.rti.ws.model.PropertyTaxNoDuesRestDTO;
...
@Controller
public class PropertyTaxNoDuesController {
//	extends MultiActionController implements InitializingBean
	private static Log log = LogFactory.getLog(PropertyTaxNoDuesController.class);
...
//	private static String const_SortBy = "SortBy";
	...
	@RequestMapping("/rtsapplication/savePropertyTaxNoDues.do")
	public ModelAndView savePropertyTaxNoDues(HttpServletRequest request, HttpServletResponse response,
			PropertyTaxNoDues propertyTaxNoDues) throws ServletException, Exception {

		HttpSession session = request.getSession();
		SessionUser sessionUser = (SessionUser) session.getAttribute("SessionUser");

		if (log.isDebugEnabled()) {
			log.debug("Invoking Property Tax No Dues");
		}
		if (log.isDebugEnabled()) {
		...
		if (mobileNumber != null) {
			...
			if (propertyTaxNoDues != null) {

				RTIApplication rtiApplication = new RTIApplication();
				PropertyTaxNoDuesRestDTO propertyTaxNoDuesRestDTO = new PropertyTaxNoDuesRestDTO();
				...
				rtiApplication.setZone(PropertyTaxPTNDetailsController.getZoneNo(propertyTaxNoDues.getZoneNo()));
				rtiApplication.setUserRegistrationDetails(user);
				rtiApplication.setIsApleSarkarApp(0);


				// rtiApplication.setApplicationCost(propertyTaxNoDues.getFeesApplicable());

				rtiApplication.setApplicantName(propertyTaxNoDues.getName());
				rtiApplication.setPdfUploadFromPortal(propertyTaxNoDues.getFilesPath());
				...
				if (savedRti.getRtiApplicationId() > 0) {
					...
					if ((retunRti != null) && (birth_regis_id > 0)) {
						propertyTaxNoDuesRestDTO.setResponseStatus("Requested data saved successfully");
						propertyTaxNoDuesRestDTO.setResponse(retunRti.getRtiApplicationId());
						propertyTaxNoDuesRestDTO.setResult(retunRti.getRtiApplnNumber());
					}
...
//				ModelAndView modelAndView = new ModelAndView("newRTIPropertyTaxNoDuesSuccess", "propertyTaxNoDues",
//						propertyTaxNoDues);
//
//				return modelAndView;
				if(as_status!=null && user_name==null) {
					...
					log.debug("aple sarkar serviceId ="+serviceId);
						...
						String UD5="NA";
						String CheckSum="NA";
						String mobileApla = propertyTaxNoDues.getMobileNo();
						 ...
						 */
						 ...
						 String retrurnurl="https://nmcnagpur.gov.in/RTSservices/ws/rtsapplication/newRtSPropertyTaxNoDuesSuccess.do?rtsAppNo="+appId+ "&isApleSarkar="+ retunRti.getIsApleSarkarApp();
					...
					return new ModelAndView("redirect:/ws/rtsapplication/newRtSPropertyTaxNoDuesSuccess.do?rtsAppNo="+retunRti.getRtiApplnNumber()
	...
	@RequestMapping("ws/rtsapplication/newRtSPropertyTaxNoDuesSuccess.do")
	public ModelAndView newRtSPropertyTaxNoDuesSuccess(HttpServletRequest request,HttpServletResponse response) {
		
		String appNo = request.getParameter("rtsAppNo");
		String isApleSarkar=request.getParameter("isApleSarkar");

		ModelAndView model = new ModelAndView("newRTIPropertyTaxNoDuesSuccess");
		
		model.addObject("propertyTaxNoDues",appNo);
...
//			HttpServletResponse response, PropertyTaxNoDues propertyTaxNoDues)
//			throws ServletException, Exception {
//		String filePath = request.getParameter("filePath");
//		long apprefid = Long.parseLong(request.getParameter("id")); 
//		RTIApplication rtsapplication;
//		propertyTaxNoDues = propertyTaxNoDuesService.get(apprefid);
//		if (propertyTaxNoDues !=null) {
//			apprefid = propertyTaxNoDues.getRti_ref_id();
//			rtsapplication = rtiApplicationService.get(propertyTaxNoDues.getRti_ref_id());
//			rtsapplication.setPdfFilesSavedPath(filePath);
//			rtiApplicationService.save(rtsapplication);
//		}
//		return new ModelAndView("redirect:/rtsapplication/editPropertyTaxNoDues.do?rtiApplicationRefId=" + propertyTaxNoDues.getPropertyTaxNoDuesId());	
//		}
	
	
	@RequestMapping("/rtsapplication/editPropertyTaxNoDues.do")
	public ModelAndView editPropertyTaxNoDues(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, Exception {

		PropertyTaxNoDues propertyTaxNoDues = null;
		HttpSession session = request.getSession();
		System.out.println("editPropertyTaxNoDues");
		...
		if (sessionUser != null) {
			if (request.getParameter("rtiApplicationRefId") != null
				...
				propertyTaxNoDues = propertyTaxNoDuesService.get(rtiApplicationRefId);

				if (propertyTaxNoDues.getRti_ref_id() > 0) {
					rtiApplicationId = propertyTaxNoDues.getRti_ref_id();
		...
		return new ModelAndView("managePropertyTaxNoDues", "propertyTaxAssessment", propertyTaxNoDues);
	}
	
	
	@RequestMapping("/rtiapplication/createPropertyTaxNoDuesWork.do")
	public ModelAndView createPropertyTaxNoDuesWork(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, Exception {
		...
		PropertyTaxNoDues propertyTaxNoDues = propertyTaxNoDuesService.get(applicationId);
		String email = propertyTaxNoDues.getEmailid();
		String mobileNo = propertyTaxNoDues.getMobileNo();
		...
		return new ModelAndView("redirect:/rtsapplication/editPropertyTaxNoDues.do?rtiApplicationRefId="+rtirefId);
	}
