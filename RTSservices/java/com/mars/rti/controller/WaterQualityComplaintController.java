...
@Controller
public class WaterQualityComplaintController {
//	extends MultiActionController implements InitializingBean
	private static Log log = LogFactory.getLog(WaterQualityComplaintController.class);
	@Autowired
...
//	private static String const_SortBy = "SortBy";
	...
	@RequestMapping("/rtsapplication/saveWaterQualityComplaint.do")
	public ModelAndView saveWaterQualityComplaint(HttpServletRequest request, HttpServletResponse response,
			WaterQualityComplaint waterQualityComplaint) throws ServletException, Exception {
		...
		if (mobileNumber != null) {
			...
			if (waterQualityComplaint != null) {

				RTIApplication rtiApplication = new RTIApplication();
				rtiApplication.setCreatedDate(CommonUtils.getCurrentStringDateAndTime());
				rtiApplication.setRegistrationDate(CommonUtils.getCurrentStringDateAndTime());
				rtiApplication.setSubject("WATER-QUALITY-COMPLAINT");
				rtiApplication.setDepartment("WATER-DEPARTMENT");
				rtiApplication.setTemplateName("water");
				rtiApplication.setWorkFlowStatus(0);
				rtiApplication.setFinalStatus("0");
				rtiApplication.setRtiserviceid(61);
				rtiApplication.setApplicantName(waterQualityComplaint.getCINNo());
				rtiApplication.setPhoneNumber(waterQualityComplaint.getMobileNo());
				rtiApplication.setMobileNumber(waterQualityComplaint.getMobileNo());
				rtiApplication.setPdfUploadFromPortal(waterQualityComplaint.getFilesPath());
				rtiApplication.setUserRegistrationDetails(user);

				if (waterQualityComplaint.getZoneNo().isEmpty()) {
					if(waterQualityComplaint.getZoneNo() != null) {			   
						rtiApplication.setZone(
								waterQualityComplaint.getZoneNo().equals("Zone No.1 - Laxmi Nagar")|| waterQualityComplaint.getZoneNo().equals("Zone No.01 - Laxmi Nagar") ? "1" :
								waterQualityComplaint.getZoneNo().equals("Zone No.2 - Dharampeth") || waterQualityComplaint.getZoneNo().equals("Zone No.02 - Dharampeth") ? "2" :
								waterQualityComplaint.getZoneNo().equals("Zone No.3 - Hanuman Nagar")|| waterQualityComplaint.getZoneNo().equals("Zone No.03 - Hanuman Nagar") ? "3" :
								waterQualityComplaint.getZoneNo().equals("Zone No.4 - Dhantoli")|| waterQualityComplaint.getZoneNo().equals("Zone No.04 - Dhantoli") ? "4" :
								waterQualityComplaint.getZoneNo().equals("Zone No.5 - Nehru Nagar")|| waterQualityComplaint.getZoneNo().equals("Zone No.05 - Nehru Nagar") ? "5" :
								waterQualityComplaint.getZoneNo().equals("Zone No.6 - Gandhibagh")|| waterQualityComplaint.getZoneNo().equals("Zone No.06 - Gandhibagh") ? "6" :
								waterQualityComplaint.getZoneNo().equals("Zone No.7 - Satranjipura")|| waterQualityComplaint.getZoneNo().equals("Zone No.07 - Satranjipura") ? "7" :
								waterQualityComplaint.getZoneNo().equals("Zone No.8 - Lakadganj")|| waterQualityComplaint.getZoneNo().equals("Zone No.08 - Lakadganj") ? "8" :
								waterQualityComplaint.getZoneNo().equals("Zone No.9 - Ashi Nagar")|| waterQualityComplaint.getZoneNo().equals("Zone No.09 - Ashi Nagar") ? "9" :
								waterQualityComplaint.getZoneNo().equals("Zone No.10 - Mangalwari")|| waterQualityComplaint.getZoneNo().equals("Zone No.010 - Mangalwari") ? "10" :
							    "");
						}
				...
				rtiApplication.setEmail(waterQualityComplaint.getEmail());
				rtiApplication.setApplicantName(waterQualityComplaint.getCINNo());
				rtiApplication.setPdfUploadFromPortal(waterQualityComplaint.getFilesPath());
				...
				if (savedRti.getRtiApplicationId() > 0) {
					...
					waterQualityComplaint.setRtiapplrefno(rti.getRtiApplnNumber());
					waterQualityComplaint.setRti_ref_id(savedRti.getRtiApplicationId());
					long water = waterQualityComplaintService.waterQualityComplaint(waterQualityComplaint);
					rti.setRtiApplicationRefId(water);
					 retunRti = rtiApplicationService.merge(rti);
					
						String requestNo = waterQualityComplaint.getRtiapplrefno();
						LocalDateTime currentDateTime = LocalDateTime.now();
						DateTimeFormatter format = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
						String dateTime = format.format(currentDateTime);
						String title = waterQualityComplaint.getTitle();
						String contractAccountNo = waterQualityComplaint.getCINNo();
						String house_no = waterQualityComplaint.getPlotNo();
						String building_name = waterQualityComplaint.getBuildingName();
						String landmark = waterQualityComplaint.getLandmark();
						String locality = waterQualityComplaint.getAreaName();
						String zone = waterQualityComplaint.getZoneNo();
						String emailId = waterQualityComplaint.getEmail();
						String Nearest_location = waterQualityComplaint.getStreetName();
						String name = waterQualityComplaint.getFullName();
						String contactNo = waterQualityComplaint.getMobileNo();
						String esr = waterQualityComplaint.getEsr();
						String problem = waterQualityComplaint.getProblem();
					...
					String applname = waterQualityComplaint.getFullName();
					...
					msg.replace("var3", "");
					String mobile = waterQualityComplaint.getMobileNo();
					SendSMS.sendSingleSMS("1507167462244373944", "NMCGov", mobile, msg);
					SendEmail.sendEmail(waterQualityComplaint.getEmail(),
							"Application Submitted Successfully" + applnNo, msg);
				}
//				ModelAndView modelAndView = new ModelAndView("newRTSWaterQualityComplaintSuccess",
//						"waterQualityComplaint", waterQualityComplaint);
//				return modelAndView;
				
				return new ModelAndView("redirect:/ws/rtsapplication/newRtSWaterQualityComplaintSuccess.do?rtsAppNo="+retunRti.getRtiApplnNumber());
	...
	@RequestMapping("ws/rtsapplication/newRtSWaterQualityComplaintSuccess.do")
	public ModelAndView newRtSWaterQualityComplaintSuccess(HttpServletRequest request,HttpServletResponse response) {
		
		String appNo = request.getParameter("rtsAppNo");
		ModelAndView model = new ModelAndView("newRTSWaterQualityComplaintSuccess");
		
		model.addObject("waterQualityComplaint",appNo);
		return model;
				
		
	}
	@RequestMapping("/rtsapplication/getWaterQualityComaplaintDetails.do")
	public void getWaterQualityComaplaintDetails(HttpServletRequest request, HttpServletResponse response) throws ServletException, Exception {
...
//	}
	...
	public ModelAndView editWaterQualityComplaint(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("rtiApplicationRefId") != null
			...
			waterQualityComplaint = waterQualityComplaintService.get(rtiApplicationRefId);
			...
			if (waterQualityComplaint.getRti_ref_id() > 0) {
				rtiApplicationId = waterQualityComplaint.getRti_ref_id();
		...
		ModelAndView modelAndView = new ModelAndView("manageWaterQualityComplaint", "waterQualityComplaint",
				waterQualityComplaint);
	...
	//
	@RequestMapping("/rtiapplication/createWaterQualityComplaintWork.do")
	public ModelAndView createWaterQualityComplaintWork(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, Exception {
		...
		WaterQualityComplaint waterQualityComplaint = waterQualityComplaintService.get(applicationId);
		String email = waterQualityComplaint.getEmail();
		String mobileNo = waterQualityComplaint.getMobileNo();
	...
	//Save Manage Disconnection
			...
			@RequestMapping("/rtsapplication/saveQualityComplaintWater.do")
			public ModelAndView saveQualityComplaintWater(HttpServletRequest request,HttpServletResponse response,WaterQualityComplaint waterQualityComplaint)
					throws ServletException, Exception {
			    String fees = request.getParameter("demandfees");
	            String refid =request.getParameter("waterQualityComplaintId");
				long apprefid = 0;
				apprefid = Long.parseLong(refid);

				waterQualityComplaint = waterQualityComplaintService.get(apprefid);
//				waterDisconnection.setSendDemandStatus(1);

				if (waterQualityComplaint.getDemandfees() == null && fees!=null) {
					waterQualityComplaint.setDemandfees(fees);
					waterQualityComplaint = waterQualityComplaintService.merge(waterQualityComplaint);

				}
				return new ModelAndView("redirect:/rtsapplication/editWaterQualityComplaint.do?rtiApplicationRefId=" + apprefid + "&fees=" + fees);	
				}

	
			@RequestMapping("/rtsapplication/saveQualityComplaintWaterPayment.do")
			public ModelAndView saveQualityComplaintWaterPayment(HttpServletRequest request,HttpServletResponse response,WaterQualityComplaint waterQualityComplaint)
					throws ServletException, Exception {
			    String paymentStatus = request.getParameter("paymentStatus");
	            String refid =request.getParameter("waterQualityComplaintId");
				long apprefid = 0;
				...
				waterQualityComplaint = waterQualityComplaintService.get(apprefid);
//				waterDisconnection.setSendDemandStatus(1);
				String status=waterQualityComplaint.getPaymentStatus();
				if (waterQualityComplaint.getPaymentStatus() == null && paymentStatus!=null) {
					waterQualityComplaint.setPaymentStatus(paymentStatus);
					waterQualityComplaint = waterQualityComplaintService.merge(waterQualityComplaint);

				}
				return new ModelAndView("redirect:/rtsapplication/editWaterQualityComplaint.do?rtiApplicationRefId=" + apprefid + "&paymentStatus=" + paymentStatus);	
				}
