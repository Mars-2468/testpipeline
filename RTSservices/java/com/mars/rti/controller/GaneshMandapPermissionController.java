...
@Controller
public class GaneshMandapPermissionController {
	...
	@Autowired
	private GaneshMandapPermissionService mandapPermissionService;
	@Autowired
...
//	
//	@Autowired
//	private RTSApplicationNumberGenerator applicationNumberGenerator;
	...
	// Ganesh Mandap Permission
		@RequestMapping("wss/rtsapplication/newRTSGaneshApplication.do")
...
//			}
	...
	@RequestMapping("/rtsapplication/saveGaneshMandapPermission.do")
	public ModelAndView saveGaneshMandapPermission(HttpServletRequest request, HttpServletResponse response,
			GaneshMandapPermission mandapPermission) throws ServletException, Exception {
...
//			Object as_status = session.getAttribute("as_status");
			...
			if (mandapPermission != null) {
				...
				//rtiApplication.setYear(String.valueOf(Year.now()));
				/*
				 ...
				 * double totalgatelength = gateLength * gateBreadth; double totalGateFees =
				 * (totalgatelength <= 9) ? 250 : 500; double cleaningfess = 500.0;
				 * mandapPermission.setCleaningfees(cleaningfess);
				 * 
				 * rtiApplication.setApplicationCost(totalGateFees + cleaningfess);
				 */
				rtiApplication.setApplicationCost(0.0);
...
//				}
				...
				//rtiApplication.setRtiApplnNumber(applicationNumberGenerator.getApplicationNumber(rtiApplication.getRtiserviceid(),"RTS/GMP/"));
				RTIApplication savedRti = rtiApplicationService.merge(rtiApplication);
				RTIApplication retunRti = null;
				if (savedRti.getRtiApplicationId() > 0) {

					RTIApplication rti = rtiApplicationService.get(savedRti.getRtiApplicationId());
					//getting Count based on current year
					
					rti.setRtiApplnNumber("RTS/GMP" + "/" + Year.now()+"/"+savedRti.getRtiApplicationId() );
					mandapPermission.setRtiapplrefno(rti.getRtiApplnNumber());
					mandapPermission.setRti_ref_id(savedRti.getRtiApplicationId());
					long birth_regis_id = mandapPermissionService.ganeshMandapPermission(mandapPermission);
					...
					// ModelAndView("redirect:/ws/search.do?rtsNo="+retunRti.getRtiApplnNumber());
					// return model;
...
//						"newRTSGaneshMandapPermissionSucess",
...
//				} else {
					return new ModelAndView(
							"redirect:/ws/rtsapplication/newRtSGaneshMandapPermissionSuccess.do?rtsAppNo="
		 ...
		 */
			...
			return new ModelAndView("errorPage").addObject("message", "Unable to save Ganesh Mandap Permission");
	}

	@RequestMapping("ws/rtsapplication/newRtSGaneshMandapPermissionSuccess.do")
	public ModelAndView newRtSGaneshMandapPermissionSuccess(HttpServletRequest request, HttpServletResponse response) {
		...
		ModelAndView model = new ModelAndView("newRTSGaneshMandapPermissionSucess");
...
//	@RequestMapping("/ws/rtsapplication/paymentSucess.do")
...
//			ModelAndView model = new ModelAndView(
//					"newRTSGaneshMandapPermissionSucess", "mandapPermission",
//					rts);
//			return model;
//		} else {
//			RTIApplication rtsapp = rtiApplicationService
...
//	}
	...
	@RequestMapping("/rtiapplication/editGaneshMandapPermission.do")
	public ModelAndView editGaneshMandapPermission(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, Exception {
		GaneshMandapPermission mandapPermission = null;
		HttpSession session = request.getSession();
		...
		if (sessionUser != null) {
			...
			ModelAndView modelAndView = new ModelAndView("manageGaneshMandapPermission", "mandapPermission",
					mandapPermission);
	...
	@RequestMapping("/rtsapplication/saveGaneshMandap.do")
	public ModelAndView saveGaneshMandap(HttpServletRequest request, HttpServletResponse response,
			GaneshMandapPermission gneshMandapPermission) throws ServletException, Exception {
	    ...
	    String reupload=request.getParameter("rp");
		...
		if (refid != null) {
			...
			if (gneshMandapPermission.getFees() == 0.0 && fees != null) {
				...
				return new ModelAndView("redirect:/rtiapplication/editGaneshMandapPermission.do?rtiApplicationRefId="
						+ apprefid + "&fees=" + fees);
			...
			if(reupload!=null && !reupload.isEmpty()) {
				if(reupload.equals("1")) {
					gneshMandapPermission.setPoliceNoc(null);
				}else if(reupload.equals("2")) {
					gneshMandapPermission.setTrafficNoc(null);
				}else if(reupload.equals("3")) {
					gneshMandapPermission.setFireNoc(null);
				}
				gneshMandapPermission.setReUploadFlag(reupload);
				gneshMandapPermission = mandapPermissionService.merge(gneshMandapPermission);
			}
			
			if(gneshMandapPermission.getPloiceremarks() !=null && gneshMandapPermission.getFireremarks()!=null) {
				
				RTIApplication rtiApplication = rtiApplicationService.get(gneshMandapPermission.getRti_ref_id());
		...
		return new ModelAndView(
				"redirect:/rtiapplication/editGaneshMandapPermission.do?rtiApplicationRefId=" + apprefid);

	}

	 
	/*
	 * @RequestMapping("/rtsapplication/createGaneshMandapPermissionWork.do") public
	 * ModelAndView createGaneshMandapPermissionWork(HttpServletRequest request,
	 * HttpServletResponse response) throws ServletException, Exception { String
	 * rtiApplicationIdString =
	 * request.getParameter(WorkflowConstants.WORKFLOW_ENTITYID); long
	 * rtiApplicationId = Long.parseLong(rtiApplicationIdString); RTIApplication
	 * rtiApplication = rtiApplicationService.get(rtiApplicationId);
	 * 
	 ...
	 * 
	 * String id = request.getParameter("id"); long applicationId =
	 * Long.parseLong(id); GaneshMandapPermission mandapPermission =
	 * mandapPermissionService.get(applicationId); String email =
	 * mandapPermission.getEmail(); String mobileNo =
	 * mandapPermission.getMobileNo(); String rtiApplnNumber =
	 * rtiApplication.getRtiApplnNumber(); String applicantName =
	 * rtiApplication.getApplicantName(); long rtirefId =
	 * rtiApplication.getRtiApplicationRefId();
	 * 
	 * if (mandapPermission != null) { if (workflowProcessDetails != null) { String
	 * processId = workflowProcessDetails.getProcessId(); if (processId != null) {
	 * processDetails =
	 ...
	 * request.setAttribute("personalTask", personalTask); }
	 * 
	 * rtiApplication.setWorkFlowStatus(2);//free service
	 * rtiApplicationService.save(rtiApplication);
	 * //SendSMSEmailController.sendPayment(applicantName, mobileNo, rtiApplnNumber,
	 * email);
	 * 
	 ...
	 * 
	 * } if (task != null && sessionUser.getUserName().equals(task.getAssignee())) {
	 * request.setAttribute("userAccess", 1); request.setAttribute("workFlowTask",
	 * task); } } else if
	 * (request.getParameter("WORKFLOW_TRANSISTION").equals("Close-Application")) {
	 * 
	 * rtiApplication.setWorkFlowStatus(1); String workflow_comments =
	 * request.getParameter(WorkflowConstants.WORKFLOW_COMMENTS);
	 ...
	 * 
	 * request.setAttribute("workFlowCompleted", "1"); } else if
	 * (request.getParameter("WORKFLOW_TRANSISTION").equals("Reject")) {
	 * rtiApplication.setWorkFlowStatus(5); String workflow_comments =
	 * request.getParameter(WorkflowConstants.WORKFLOW_COMMENTS);
	 * rtiApplication.setWorkflowComments(workflow_comments);
	 * 
	 * rtiApplicationService.save(rtiApplication);
	 ...
	 * rtiApplnNumber, email);
	 * 
	 * request.setAttribute("workFlowCompleted", "1"); } } else {
	 * session.setAttribute("rejected", 5); rtiApplication.setWorkFlowStatus(5);
	 ...
	 * ModelAndView("redirect:/rtsapplication/editGaneshMandapPermission.do"); }
	 */
	 
	
	
	
	@RequestMapping("/rtsapplication/createGaneshMandapPermissionWork.do")
	public ModelAndView createGaneshMandapPermissionWork(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, Exception {
		...
		RTIApplication rtiApplication = null;
		  HttpSession session = request.getSession();
		if(cid!=null) {
			String rtiApplicationIdString= request.getParameter("rtiApplicationRefIds");
			if(rtiApplicationIdString!=null && !rtiApplicationIdString.isEmpty()) {
				 rtiApplicationId = Long.parseLong(rtiApplicationIdString);
				 rtiApplication = rtiApplicationService.get(rtiApplicationId);
				GaneshMandapPermission mandapPermission = mandapPermissionService.get(rtiApplication.getRtiApplicationRefId());
				String email = mandapPermission.getEmail();
				String mobileNo = mandapPermission.getMobileNo();
				String rtiApplnNumber = rtiApplication.getRtiApplnNumber();
				String applicantName = rtiApplication.getApplicantName();
			    rtirefId = rtiApplication.getRtiApplicationRefId();
				...
				if(cid.equals("1")) {
					...
					rtiApplication.setWorkFlowStatus(5);//reject
					rtiApplicationService.save(rtiApplication);
		...
		return new ModelAndView("redirect:/rtiapplication/editGaneshMandapPermission.do");
	}
