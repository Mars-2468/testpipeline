...
public class StateDAOImpl extends HibernateDaoSupport implements StateDAO {
	...
	public State get(long id) {
		try {
			...
			return (State) getHibernateTemplate().get(State.class,new Long(id));
    ...
    public State merge(State entity) {
    	try {
		    ...
		    return (State) getHibernateTemplate().merge(entity);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void delete(long id) {
        try {
        	...
        	State entity = (State) getHibernateTemplate().load(State.class, new Long(id));
	...
	public List<State> getAll() {
		try {
			if (log.isDebugEnabled()) {
				log.debug("get all Method is called for State");
			}
    ...
    public void deleteAll(List<State> listState)
    {
		try
		{
	    	if (log.isDebugEnabled())
	   		{
				log.debug("deleteAll Method is called for State");
	    	}
	    	getHibernateTemplate().deleteAll(listState);
	    	log.debug("deleted successfuly a list of State records from database");
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void saveAll(List<State> listState)
    {
		try
		{
	    	...
	    	getHibernateTemplate().saveOrUpdateAll(listState);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
	...
	{
		try {
			String fromClause = "select state from State state";
	...
	public List<State> getStateListBySearch(StateSearch searchState){
		...
		try
		{
			long pageNumber=searchState.getCurrentPage();
			String orderBy=searchState.getOrderBy();
			String sortBy=searchState.getSortBy();
			String name=CommonUtils.getEscapedSQLString(searchState.getName());
			...
			if((name!=null && name.length()>0)){
				if(!flag){
				    ...
				    sql.append("upper(state.name) like upper('%"+ name +"%')");
			}
	...
	public long getStateCount(){
	    try {
			String fromClause = "select count(*) from State state";
			Query query = getSession().createQuery(fromClause);
	...
	public long getStateCountBySearch(StateSearch searchState){
		...
		try{
            ...
            StringBuffer sql=new StringBuffer();
            String fromClause="select count(state) from State state";
            List list=null;
            boolean flag=false;
	...
	public List<State> findByProperty(String propertyName, Object value) 
    {
		try 
	  	{
		  	log.info("finding State instance with property: "+ propertyName + ", value: " + value);
			String queryString = "select state from State state where state."+ propertyName + "= ?";
	...
	public State checkForDuplicates(State state)
	{
		...
		try {
			String fromClause = "select state from State state where " ;

			if(state.getStateId() > 0)		
				fromClause = fromClause +" stateId <> "+state.getStateId()+" and ";
			
			//Sample Code update accordingly
			//fromClause = fromClause +" state.{Many-to-one}.{Many-to-one}Id = "+state.get{Many-to-one}().get{Many-to-one}Id()+" and ";
			fromClause = fromClause +" (upper(name) = upper('"+ CommonUtils.getEscapedSQLString(state.getName()) +"')) " ;
