 ...
 * <p>Title: CityCategoryMasterDaoImpl.java </p>

 * @see com.mars.pis.model.CityCategoryMaster
 
 * <p>Description: This class is used for hibernate operations for CityCategoryMaster module</p>
 ...
 */
 ...
 @Repository("CityCategoryDAO")
 public class CityCategoryDAOImpl extends HibernateDaoSupport implements CityCategoryDAO {

    private static final Log log = LogFactory.getLog(CityCategoryDAOImpl.class);
	 ...
	 */
	 ...
	 public CityCategory get(long id) {
		try {
			...
			return (CityCategory) getHibernateTemplate().get(CityCategory.class,new Long(id));
	 ...
	 */
	 ...
	 public CityCategory merge(CityCategory entity) {
    	try {
		    ...
		    return (CityCategory) getHibernateTemplate().merge(entity);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace(),exception);
	    	exception.printStackTrace();
	 ...
	 */
	 ...
	 public void delete(long id) {
        try {
    		if(log.isDebugEnabled()){
        		log.debug("deleting CityCategoryMaster instance");
        	}
        	CityCategory entity = (CityCategory) getHibernateTemplate().load(CityCategory.class, new Long(id));
	 ...
	 */
	 ...
	 public List<CityCategory> getAll() {
		try {
			if (log.isDebugEnabled()) {
				log.debug("get all Method is called for CityCategory");
			}
			return getHibernateTemplate().loadAll(CityCategory.class);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace(),exception);
	    	exception.printStackTrace();
	 ...
	 public void deleteAll(List<CityCategory> listCityCategory)
    {
		try
		{
	    	if (log.isDebugEnabled())
	   		{
				log.debug("deleteAll Method is called for CityCategory");
	    	}
	    	getHibernateTemplate().deleteAll(listCityCategory);
	    	log.debug("deleted successfuly a list of CityCategory records from database");
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace(),exception);
	    	exception.printStackTrace();
	 ...
	 public void saveAll(List<CityCategory> listCityCategory)
    {
		try
		{
	    	...
	    	getHibernateTemplate().saveOrUpdateAll(listCityCategory);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace(),exception);
	    	exception.printStackTrace();
	...
	{
		try {
			String fromClause = "select cityCategory from CityCategory cityCategory";
			...
			List<CityCategory> list = query.list();
	 ...
	 */
	 ...
	 public long getCityCategoryCount(){
	    try {
			String fromClause = "select count(*) from CityCategory cityCategory";
			Query query = getSession().createQuery(fromClause);
	 ...
	 public List<CityCategory> getEntityListBySQLQuery(String strSql, Hashtable<String, NullableType> hstDataTypes)
	{
		try{
			...
			query.addEntity(CityCategory.class);
			return query.list();
		} catch (Exception exception)
		{
			log.error(exception.getStackTrace(),exception);
	 ...
	 public List<CityCategory> findByProperty(String propertyName, Object value) 
    {
		try 
	  	{
		  	log.info("finding CityCategory instance with property: "+ propertyName + ", value: " + value);
			String queryString = "select cityCategory from CityCategory cityCategory where cityCategory."+ propertyName + "= ?";
	 ...
	 * 
	 * @param CityCategoryMaster as cityCategory
	 * @return CityCategory as model object
	 */
	 ...
	 public CityCategory checkForDuplicates(CityCategory cityCategory)
	{
		...
		try {
			String fromClause = "select cityCategoryMaster from CityCategoryMaster cityCategoryMaster where " ;

			if(cityCategory.getCityCategoryId() > 0)		
				fromClause = fromClause +" cityCategoryMasterId <> "+cityCategory.getCityCategoryId()+" and ";
			
			//Sample Code update accordingly
			//fromClause = fromClause +" cityCategoryMaster.{Many-to-one}.{Many-to-one}Id = "+cityCategoryMaster.get{Many-to-one}().get{Many-to-one}Id()+" and ";
			//fromClause = fromClause +" (upper(name) = upper('"+ CommonUtils.getEscapedSQLString(cityCategoryMaster.getName()) +"')) " ;
		
			Query query = getSession().createQuery(fromClause);		
			List list = query.list();

			if (list != null && list.size() > 0) 
				return (CityCategory) list.get(0);
