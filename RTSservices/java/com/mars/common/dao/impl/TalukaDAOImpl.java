...
public class TalukaDAOImpl extends HibernateDaoSupport implements TalukaDAO {

    private static final Log log = LogFactory.getLog(TalukaDAOImpl.class);
	...
	/**
	 * This method returns Taluka object for a given Id
	 * 
	 * @param id as long
	...
	public Taluka get(long id) {
		try {
			...
			return (Taluka) getHibernateTemplate().get(Taluka.class,new Long(id));
    ...
    /**
	 * This method save/updates the Taluka information to the database.
	 * 
	 * @param Taluka object
	 * 
	 */
    ...
    public Taluka merge(Taluka entity) {
    	try {
		    ...
		    return (Taluka) getHibernateTemplate().merge(entity);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void delete(long id) {
        try {
    		if(log.isDebugEnabled()){
        		log.debug("deleting Taluka instance");
        	}
        	Taluka entity = (Taluka) getHibernateTemplate().load(Taluka.class, new Long(id));
	...
	public List<Taluka> getAll() {
		try {
			if (log.isDebugEnabled()) {
				log.debug("get all Method is called for Taluka");
			}
			return getHibernateTemplate().loadAll(Taluka.class);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void deleteAll(List<Taluka> listTaluka)
    {
		try
		{
	    	if (log.isDebugEnabled())
	   		{
				log.debug("deleteAll Method is called for Taluka");
	    	}
	    	getHibernateTemplate().deleteAll(listTaluka);
	    	log.debug("deleted successfuly a list of Taluka records from database");
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
    ...
    public void saveAll(List<Taluka> listTaluka)
    {
		try
		{
	    	...
	    	getHibernateTemplate().saveOrUpdateAll(listTaluka);
		} catch (Exception exception)
		{
	    	log.error(exception.getStackTrace());
	    	exception.printStackTrace();
	...
	{
		try {
			String fromClause = "select taluka from Taluka taluka";
	...
	public List<Taluka> getTalukaListBySearch(TalukaSearch searchTaluka){
		...
		try{
			long pageNumber=searchTaluka.getCurrentPage();
			String orderBy=searchTaluka.getOrderBy();
			String sortBy=searchTaluka.getSortBy();
			
			String name=CommonUtils.getEscapedSQLString(searchTaluka.getName());
			long districtId=searchTaluka.getDistrictId();
			
			StringBuffer sql=new StringBuffer();
			String fromClause="select taluka from Taluka taluka";
			List<Taluka> list=null;
			boolean flag=false;
			...
			if(name!=null && name.length()>0){
				if(!flag){
				    ...
				    sql.append("upper(taluka.name) like upper('%"+ name +"%')");
			}
			if(districtId>0){
				if(!flag){
					flag=true;
				    ...
				    sql.append("taluka.district.districtId=" + districtId);
			}
	...
	public long getTalukaCount(){
	    try {
			String fromClause = "select count(*) from Taluka taluka";
			Query query = getSession().createQuery(fromClause);
	...
	public long getTalukaCountBySearch(TalukaSearch searchTaluka){
		...
		try{
			...
			StringBuffer sql=new StringBuffer();
			String fromClause="select count(taluka) from Taluka taluka";
			List list=null;
			boolean flag=false;
	...
	public List<Taluka> findByProperty(String propertyName, Object value) 
    {
		try 
	  	{
		  	log.info("finding Taluka instance with property: "+ propertyName + ", value: " + value);
			String queryString = "select taluka from Taluka taluka where taluka."+ propertyName + "= ?";
	...
	public Taluka checkForDuplicates(Taluka taluka)
	{
		...
		try {
			String fromClause = "select taluka from Taluka taluka where " ;

			if(taluka.getTalukaId() > 0)		
				fromClause = fromClause +" talukaId <> "+taluka.getTalukaId()+" and ";
			
			//Sample Code update accordingly
			//fromClause = fromClause +" taluka.{Many-to-one}.{Many-to-one}Id = "+taluka.get{Many-to-one}().get{Many-to-one}Id()+" and ";
			fromClause = fromClause +" (upper(name) = upper('"+ CommonUtils.getEscapedSQLString(taluka.getName()) +"')) and " ;
			fromClause = fromClause +" district.districtId = "+ taluka.getDistrict().getDistrictId();
		
			Query query = getSession().createQuery(fromClause);		
			List list = query.list();

			if (list != null && list.size() > 0) 
				return (Taluka) list.get(0);
